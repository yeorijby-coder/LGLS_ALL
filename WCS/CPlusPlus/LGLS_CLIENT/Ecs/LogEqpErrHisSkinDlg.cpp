...
BEGIN_MESSAGE_MAP(CLogEqpErrHisSkinDlg, CSkinDialog)
	ON_MESSAGE(SSM_CLICK, &CLogEqpErrHisSkinDlg::OnLglsHdrSort)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_SIZE()
	ON_WM_CLOSE()
...
// [LGLS] 그리드 헤더 클릭 정렬
LRESULT CLogEqpErrHisSkinDlg::OnLglsHdrSort(WPARAM wParam, LPARAM lParam)
{
...
#pragma endregion
	...
	{ CString _tt; GetWindowText(_tt); if(_tt.Find(_T("[")) < 0) SetWindowText(_tt + _T(" [EQP_ERR_HIS]")); }	// [LGLS] 제목에 조회 테이블명
	return TRUE;
}
...
void CLogEqpErrHisSkinDlg::RenameResource( EN_LANG enLang)
{
	...
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_EQP_TYP, strValue);
	...
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_EQP_NO, strValue);
	...
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_BTN_EQP_HIS_SEARCH, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_eqphislog\\"), _T("dlg_eqphislog"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("search"), (int)enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_EQP_HIS_RESULT, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_eqphislog\\"), _T("dlg_eqphislog"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("select"), (int)enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_GRP_EQP_ERR_HIS_SEARCH, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_eqphislog\\"), _T("dlg_eqphislog"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("selectresult"), (int)enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_GRP_EQP_ERR_HIS_SEARCH2, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_eqphislog\\"), _T("dlg_eqphislog"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("eqperrcd"), (int)enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_EQP_ERR_CD, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_eqphislog\\"), _T("dlg_eqphislog"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("eqperrmsg"), (int)enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_EQP_ERR_MSG, strValue);
}
...
CString CLogEqpErrHisSkinDlg::GetQrySelect()
{
	...
	strSql += CRLF + _T("    AND ( ERROR_DT >= CONVERT(DATETIME, STUFF(STUFF(STUFF('") + strdtFrom + _T("',13,0,':'),11,0,':'),9,0,' '))");	// [LGLS]
	strSql += CRLF + _T("    AND   ERROR_DT <=  CONVERT(DATETIME, STUFF(STUFF(STUFF('") + strdtTo + _T("',13,0,':'),11,0,':'),9,0,' ')) )");	// [LGLS]
...
CString CLogEqpErrHisSkinDlg::GetQrySelect_Main(int nRowCheck,BOOL bSearch)
{
	...
	strSql += CRLF + _T("   AND (CONVERT(VARCHAR(8),EEH.ERROR_DT,112) + REPLACE(CONVERT(VARCHAR(8),EEH.ERROR_DT,108),':','')) BETWEEN '") + strdtFrom + _T("' AND '") + strdtTo + _T("'");	// [LGLS]
