...
int CEcsLayout::m_nLeftInsetS = 0;		// [LGLS 2026-09-09] 왼쪽 범례 폭
...
BOOL CEcsLayout::Test(CWnd* pWnd, int nLeft, int nRight, int nTop, int nBottom)
{
	...
	// [LGLS 2026-07-22] 검색 확대 로직 제거: 좌우가 뒤집힌 뷰포트(l=nRight-15, r=nLeft+15)를 설정해
	//   레이아웃 전체가 축소/왜곡되던 버그. 검색 위치는 하이라이트 색(SetSeachColor)으로 표시하고
	//   레이아웃 배율은 그대로 유지한다.
