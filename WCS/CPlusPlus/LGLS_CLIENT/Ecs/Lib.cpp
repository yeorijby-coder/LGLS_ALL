...
CString CLib::GetObsAddr(CString pstrOwner, CString pstrName)
{
	...
	{
		s_bLoaded = TRUE;
		TCHAR chrFile[MAX_PATH] = {0};
		GetModuleFileName(NULL, chrFile, MAX_PATH);
		CString strPath(chrFile);
		strPath = strPath.Left(strPath.ReverseFind('\\')) + _T("\\observables.tsv");
		CStdioFile f;
		if (f.Open(strPath, CFile::modeRead | CFile::typeText))
		{
			...
			{
				// UTF-8 BOM 등 머리의 비ASCII 제거
				while (!line.IsEmpty() && (line[0] < 'A' || line[0] > 'Z')) line.Delete(0);
				int nT1 = line.Find('\t'); if (nT1 < 0) continue;
				int nT2 = line.Find('\t', nT1 + 1); if (nT2 < 0) continue;
				CString strOwner = line.Left(nT1);
				CString strName  = line.Mid(nT1 + 1, nT2 - nT1 - 1);
				int nDev = line.Find(_T("DEVICE_TYPE="));
				int nAdr = line.Find(_T("ADDRESS_NO="));
				if (nDev < 0 || nAdr < 0) continue;
				TCHAR chDev = line[nDev + 12];
				CString strAdr;
				for (int k = nAdr + 11; k < line.GetLength(); k++)
				{
					TCHAR c = line[k];
					if ((c >= '0' && c <= '9') || (c >= 'A' && c <= 'F') || (c >= 'a' && c <= 'f')) strAdr += c;
					else break;
				}
				if (strAdr.IsEmpty()) continue;
				TCHAR chView = (chDev == 'W') ? 'D' : (chDev == 'B') ? 'M' : (chDev == 'R') ? 'R' : '?';
				if (chView == '?') continue;
...
// [LGLS 2026-08-04] 조회 조건 입력칸(라벨+편집) 1쌍을 **보이는 행의 바로 아래**에 런타임 생성한다.
//   숨겨둔 예비 컨트롤을 자리표로 쓰던 방식을 버리고, 실제 보이는 행들의 좌표로 계산한다.
//     nLblAboveRow / nLblRow : 행 높이를 구할 두 라벨(위 행 -> 기준 행)
//     nValRow                : 열 위치(x/width)와 편집 스타일을 가져올 기준 행의 값 컨트롤
//   *라벨은 CStatic 이 아닌 CWnd 로 만든다 - CSkinDialog::OnCtlColor 가 IsKindOf(CStatic) 에
//    NULL 브러시를 돌려주어 배경이 다르게 칠해지기 때문.
void CLib::AddFilterField(CWnd* pDlg, int nLblAboveRow, int nLblRow, int nValRow,
                          LPCTSTR szText, int nLblId, int nEdtId, CPtrArray& arKeep)
{
	if (pDlg == NULL) return;
	CWnd* pAbove = pDlg->GetDlgItem(nLblAboveRow);
	CWnd* pLbl   = pDlg->GetDlgItem(nLblRow);
	CWnd* pVal   = pDlg->GetDlgItem(nValRow);
	if (pLbl == NULL || pVal == NULL) return;

	CRect rcAbove, rcLbl, rcVal;
	pLbl->GetWindowRect(&rcLbl); pDlg->ScreenToClient(&rcLbl);
	pVal->GetWindowRect(&rcVal); pDlg->ScreenToClient(&rcVal);

	int nPitch = rcLbl.Height() + 8;
	if (pAbove != NULL)
	{
		pAbove->GetWindowRect(&rcAbove); pDlg->ScreenToClient(&rcAbove);
		if (rcLbl.top - rcAbove.top > 0) nPitch = rcLbl.top - rcAbove.top;
	}

	// 새 행 : 라벨은 기준 라벨 그대로, 값은 기준 값의 x 범위 + 라벨의 y 범위(콤보박스여도 높이가 튀지 않게)
	CRect rcNewL = rcLbl;
	CRect rcNewV(rcVal.left, rcLbl.top, rcVal.right, rcLbl.bottom);
	rcNewL.OffsetRect(0, nPitch);
	rcNewV.OffsetRect(0, nPitch);

	DWORD dwLStyle = (pLbl->GetStyle() | WS_CHILD | WS_VISIBLE) & ~(WS_TABSTOP | WS_GROUP);
	DWORD dwLEx    = pLbl->GetExStyle();

	TCHAR szCls[64]; szCls[0] = 0;
	::GetClassName(pVal->GetSafeHwnd(), szCls, sizeof(szCls)/sizeof(szCls[0]) - 1);
	BOOL  bValEdit = (_tcsicmp(szCls, _T("Edit")) == 0);
	DWORD dwEStyle = bValEdit ? (pVal->GetStyle() | WS_CHILD | WS_VISIBLE | WS_TABSTOP)
	                          : (WS_CHILD | WS_VISIBLE | WS_TABSTOP | WS_BORDER | ES_LEFT | ES_AUTOHSCROLL);
	DWORD dwEEx    = bValEdit ? pVal->GetExStyle() : 0;

	CWnd* pNewL = new CWnd();
	pNewL->CreateEx(dwLEx, _T("STATIC"), szText, dwLStyle, rcNewL, pDlg, nLblId);
	if (pLbl->GetFont() != NULL) pNewL->SetFont(pLbl->GetFont());
	arKeep.Add(pNewL);

	CEdit* pNewE = new CEdit();
	pNewE->CreateEx(dwEEx, _T("EDIT"), _T(""), dwEStyle, rcNewV, pDlg, nEdtId);
	if (pVal->GetFont() != NULL) pNewE->SetFont(pVal->GetFont());
	arKeep.Add(pNewE);
}

// [LGLS 2026-08-01] 기준 행(라벨/값) 바로 아래에 두 행을 런타임 생성한다.
//   rc 대화상자 템플릿을 고치지 않고(리소스는 UTF-16 이라 편집 사고가 잦다) 실행 시점 좌표로 처리한다.
//   ① 기준 행 위 라벨과의 간격으로 행 높이를 구하고 ② 그룹박스 아래 컨트롤을 2행만큼 내린 뒤
//   ③ 그룹박스와 창 높이를 늘리고 ④ 새 라벨/값 컨트롤을 만든다.
void CLib::AddTwoRowsBelow(CWnd* pDlg, int nLblAbove, int nLbl, int nVal,
                           LPCTSTR szText1, int nLbl1, int nVal1,
                           LPCTSTR szText2, int nLbl2, int nVal2,
                           const int* pGrpIds, int nGrpCnt, CPtrArray& arKeep)
{
	if (pDlg == NULL) return;
	CWnd* pAbove = pDlg->GetDlgItem(nLblAbove);
	CWnd* pLbl   = pDlg->GetDlgItem(nLbl);
	CWnd* pVal   = pDlg->GetDlgItem(nVal);
	if (pLbl == NULL || pVal == NULL) return;

	CRect rcLbl, rcVal, rcAbove;
	pLbl->GetWindowRect(&rcLbl); pDlg->ScreenToClient(&rcLbl);
	pVal->GetWindowRect(&rcVal); pDlg->ScreenToClient(&rcVal);

	int nPitch = rcLbl.Height() + 4;
	if (pAbove != NULL)
	{
		pAbove->GetWindowRect(&rcAbove); pDlg->ScreenToClient(&rcAbove);
		if (rcLbl.top - rcAbove.top > 0) nPitch = rcLbl.top - rcAbove.top;
	...
	// 그룹박스 최하단(확장 전) 아래의 컨트롤을 전부 내린다.
	int nGrpBottom = 0;
	int i;
	for (i = 0; i < nGrpCnt; i++)
	{
		CWnd* pGrp = pDlg->GetDlgItem(pGrpIds[i]);
		if (pGrp == NULL) continue;
		CRect rc; pGrp->GetWindowRect(&rc); pDlg->ScreenToClient(&rc);
		if (rc.bottom > nGrpBottom) nGrpBottom = rc.bottom;
	}
	if (nGrpBottom > 0)
	{
		for (CWnd* pChild = pDlg->GetWindow(GW_CHILD); pChild != NULL; pChild = pChild->GetWindow(GW_HWNDNEXT))
		{
			CRect rc; pChild->GetWindowRect(&rc); pDlg->ScreenToClient(&rc);
			if (rc.top >= nGrpBottom - 2)
			{
				rc.OffsetRect(0, nGrow);
	...
	// 창 확장
	CRect rcWin; pDlg->GetWindowRect(&rcWin);
	pDlg->SetWindowPos(NULL, 0, 0, rcWin.Width(), rcWin.Height() + nGrow, SWP_NOMOVE | SWP_NOZORDER);

	// 새 행 생성 : 기준 행의 컨트롤 스타일을 그대로 복제해 다른 행과 똑같이 보이게 한다.
	//   *라벨은 CStatic 이 아닌 CWnd 로 만든다 - CSkinDialog::OnCtlColor 가
	//     IsKindOf(CStatic) 인 컨트롤에 NULL 브러시를 돌려주어 배경이 다르게 칠해지기 때문.
	//     rc 로 만든 라벨은 GetDlgItem 이 임시 CWnd 를 주므로 이 분기를 타지 않는다.
	//   *값은 기준 값 컨트롤(EDIT)의 스타일/확장스타일을 복제한 읽기전용 CEdit.
	CFont* pFontL = pLbl->GetFont();
	CFont* pFontV = pVal->GetFont();
	LPCTSTR szTxt[2] = { szText1, szText2 };
	int nIdL[2] = { nLbl1, nLbl2 };
	int nIdV[2] = { nVal1, nVal2 };
	...
	::GetClassName(pVal->GetSafeHwnd(), szCls, sizeof(szCls)/sizeof(szCls[0]) - 1);
	BOOL  bValEdit = (_tcsicmp(szCls, _T("Edit")) == 0);
	DWORD dwVStyle = bValEdit ? ((pVal->GetStyle() | WS_CHILD | WS_VISIBLE | ES_READONLY) & ~(WS_TABSTOP | WS_GROUP))
	                          : (WS_CHILD | WS_VISIBLE | ES_LEFT | ES_AUTOHSCROLL | ES_READONLY);
	DWORD dwVEx    = bValEdit ? pVal->GetExStyle() : WS_EX_STATICEDGE;

	for (i = 0; i < 2; i++)
	{
		CRect rcL = rcLbl; rcL.OffsetRect(0, nPitch * (i + 1));
		CRect rcV = rcVal; rcV.OffsetRect(0, nPitch * (i + 1));

		CWnd* pNewL = new CWnd();
		pNewL->CreateEx(dwLEx, _T("STATIC"), szTxt[i], dwLStyle, rcL, pDlg, nIdL[i]);
		if (pFontL != NULL) pNewL->SetFont(pFontL);
		arKeep.Add(pNewL);

		CEdit* pNewV = new CEdit();
		pNewV->CreateEx(dwVEx, _T("EDIT"), _T(""), dwVStyle, rcV, pDlg, nIdV[i]);
		if (pFontV != NULL) pNewV->SetFont(pFontV);
		arKeep.Add(pNewV);
	}
...
BOOL CLib::BindCombo_SC_NO_MANUAL(CComboBoxWrapper& cbx, CString strCDX_CD, CEcsDoc *pDoc, int nEN_LANG, CString strSC_GRP_NO, CString strSC_NO)
{
   ...
   strSql += CRLF + _T("   FROM SC_DATA_LGLS SD LEFT OUTER JOIN COMMON_CODE CCD ");
...
BOOL CLib::BindCombo_RTV_NO(CComboBoxWrapper& cbx, CEcsDoc *pDoc, int nEN_LANG)
{
   ...
   strSql += CRLF + _T("   FROM RTV_DATA_LGLS RD LEFT OUTER JOIN COMMON_CODE CCD ");
