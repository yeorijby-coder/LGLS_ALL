...
BOOL CViewSearchDlg::OnInitDialog()
{
	...
	RenameResource(pEn);
	Invalidate(TRUE);
	{ CString _tt; GetWindowText(_tt); if(_tt.Find(_T("[")) < 0) SetWindowText(_tt + _T(" [CV_DATA]")); }	// [LGLS] 제목에 조회 테이블명
...
{
	KillTimer(777);
	if(!m_strBlinkTrack.IsEmpty()){ CTrackInfo* _p=m_pDoc->GetTrackInfoNew(m_strBlinkTrack); if(_p && _p->m_pTrackCtrl) _p->SetSeachColor(_p->m_pTrackCtrl, true); m_strBlinkTrack.Empty(); }
	m_pDoc->m_pViewSearchDlg = NULL;
	CSkinDialog::OnClose();
}

void CViewSearchDlg::OnTimer(UINT_PTR nIDEvent)
{
	if(nIDEvent == 777)
	{
		CTrackInfo* p = m_strBlinkTrack.IsEmpty() ? NULL : m_pDoc->GetTrackInfoNew(m_strBlinkTrack);
		if(p == NULL || p->m_pTrackCtrl == NULL || m_nBlinkCnt <= 0)
		{
			if(p != NULL && p->m_pTrackCtrl != NULL) p->SetSeachColor(p->m_pTrackCtrl, true);	// 원복
			KillTimer(777);
			m_strBlinkTrack.Empty();
			return;
		}
		m_bBlinkOn = !m_bBlinkOn;
		p->SetSeachColor(p->m_pTrackCtrl, m_bBlinkOn ? FALSE : TRUE);	// FALSE=분홍, TRUE=원복
		m_nBlinkCnt--;
		return;
	}
...
void CViewSearchDlg::RenameResource( EN_LANG m_enLang)
{
	...
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_SEARCH_TYP, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_search\\"), _T("dlg_search"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("search"), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(ID_BTN_SEARCH, strValue);
...
void CViewSearchDlg::OnSetSearchCv()
{
	...
	// [LGLS] 검색 트랙 5초간 깜빡 후 원복
	m_strBlinkTrack = strTrack;
	m_nBlinkCnt = 10;
	m_bBlinkOn = TRUE;
	KillTimer(777);
	SetTimer(777, 500, NULL);
...
CString CViewSearchDlg::GetTrackNo(CString pTemp, CString pChkName)
{
	...
	if (pChkName == _T("0")) //TRACK_NO
	{
		...
		strSql.Format(_T("		SELECT MC_NO							\n")
					  ...
					  _T("		 LIMIT 1         						\n"), m_pDoc->m_WH_TYP, strTemp);
#elif MSSQL
					  _T(""), m_pDoc->m_WH_TYP, strTemp);	// [LGLS]
		CLib::ApplyTopN(strSql, _T("1"));	// [LGLS] TOP 1 instead of LIMIT 1
#endif
	}
	...
	else if (pChkName == _T("2")) //TRAY_NO
	{
		...
		strSql.Format(_T("		SELECT MC_NO							\n")
					  ...
					  _T("		 LIMIT 1								\n"), m_pDoc->m_WH_TYP, strTemp, strTemp);
#elif MSSQL
					  _T(""), m_pDoc->m_WH_TYP, strTemp, strTemp);	// [LGLS]
		CLib::ApplyTopN(strSql, _T("1"));	// [LGLS] TOP 1 instead of LIMIT 1
#endif
	}
