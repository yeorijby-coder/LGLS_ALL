...
void CManualJob::RenameResource( EN_LANG m_enLang)
{
	...
	if (!strValue.IsEmpty()) SetDlgItemText(ID_BTN_MANUL_JOB_COMP, strValue);
	
	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobmanual\\"), _T("dlg_jobmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("delete"), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(ID_BTN_MANUAL_JOB_CELL_DELETE, strValue);


	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobmanual\\"), _T("dlg_jobmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("manualadd "), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(ID_BTN_MANUL_JOB_INSERT, strValue);


	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobmanual\\"), _T("dlg_jobmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("manualdelete "), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(ID_BTN_MANUAL_JOB_DELETE, strValue);


	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobmanual\\"), _T("dlg_jobmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("datereset "), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(ID_BTN_MANUAL_JOB_CLEAR, strValue);


	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobmanual\\"), _T("dlg_jobmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("copy"), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(ID_BTN_MANUAL_JOB_COPY, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobmanual\\"), _T("dlg_jobmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("whtyp"), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_MANUAL_JOB_WH_TYP, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobmanual\\"), _T("dlg_jobmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("jobtyp"), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_MANUAL_JOB_JOBTYP, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobmanual\\"), _T("dlg_jobmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("productsize"), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_MANUAL_JOB_PRODUCT_SIZE, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobmanual\\"), _T("dlg_jobmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("strageloc"), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_MANUAL_JOB_LOCATION, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobmanual\\"), _T("dlg_jobmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("selectcnt"), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_MANUAL_JOB_RESULT, strValue);
...
//	SetDlgItemText(IDC_LBL_MANUAL_JOB_DURATION_TIME, strValue);
	...
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_MANUAL_JOB_STARTPOS, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobmanual\\"), _T("dlg_jobmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("destpos"), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_MANUAL_JOB_LBL_DESTPOS, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobmanual\\"), _T("dlg_jobmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("remark"), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_MANUAL_JOB_REMARKS, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobmanual\\"), _T("dlg_jobmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("jobstartdt"), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_MANUAL_JOB_STODATE, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobmanual\\"), _T("dlg_jobmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("serach"), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(ID_BTN_MANUAL_JOB_SEARCH, strValue);
	...
	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobinfo\\"), _T("dlg_jobinfo"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("select"), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_GRP_MANUAL_JOB_SEARCH, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobinfo\\"), _T("dlg_jobinfo"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("selectresult"), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_GRP_MANUAL_JOB_SEARCH2, strValue);
	...
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_MANUAL_JOB_LOCATION_TO, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobmanual\\"), _T("dlg_jobmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("jobpriority"), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_MANUAL_JOB_PRIORITY, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobmanual\\"), _T("dlg_jobmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("jobcnt"), (int)m_enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_MANUAL_JOB_CNT, strValue);
...
void CManualJob::OnBnClickedBtnManulJobInsert()
{
	...
	if(AfxMessageBox(m_pDoc->GetMsgLangDef(_T("입력한 작업을 생성하시겠습니까?")), MB_YESNO) == IDYES)
    {
		...
		// [LGLS 2026-07-19] semi-auto job types(10~15) follow the same rules as auto types:
		//   10(semi move)->6(workbench move), 11~15 -> 1~5. INSERT keeps the original code(10~15).
...
CString CManualJob::GetQryJobMst()
{
	...
	strSql.Format(_T(" SELECT '' AS CheckBox																					    \n")	
				  ...
				  _T("		, ") + _T("'[' + JM.JOB_STATUS + '] ' + ") + m_pDoc->NVL + _T("(CCD_JOB_STATUS.CCD_NM_KOR, JM.JOB_STATUS) AS JOB_STATUS										\n")	
				  ...
				  _T("		, ") + m_pDoc->NVL + _T("(CONVERT(VARCHAR(10),JM.JOB_START_DT,101) + ' ' + CONVERT(VARCHAR(8),JM.JOB_START_DT,108), '00-00-0000 00:00:00') AS JOB_START_DT	\n")	// [LGLS]
				  _T("		, ") + m_pDoc->NVL + _T("(CONVERT(VARCHAR(10),JM.INS_DT,101) + ' ' + CONVERT(VARCHAR(8),JM.INS_DT,108), '00-00-0000 00:00:00') AS INS_DT				\n")	// [LGLS]
				  ...
				  _T("    AND (CONVERT(VARCHAR(8),JM.JOB_START_DT,112) + REPLACE(CONVERT(VARCHAR(8),JM.JOB_START_DT,108),':','')) BETWEEN '") + strdtFrom + _T("' AND '") + strdtTo + _T("'"), strWH_TYP); // [LGLS]
...
void CManualJob::OnSelchangeComboManualJobEditJobTyp()
{
	...
	strJobTyp = m_cbxManualJobJobTyp.GetItemKey(m_cbxManualJobJobTyp.GetCurSel());
	// [LGLS 2026-07-19] semi-auto(10~15): same enable/disable as auto (10->6, 11~15->1~5)
	if (strJobTyp.GetLength() == 2 && strJobTyp.GetAt(0) == _T('1'))
		strJobTyp = (strJobTyp == _T("10")) ? _T("6") : CString(strJobTyp.GetAt(1));
		if (strJobTyp == _T("0"))
	{
...
CString CManualJob::GetQrySelectCELLMST(CString strBANK, CString strBAY, CString strLEV, CString& strSC_NO, CString& strCELL_USE_YN, CString& strSC_PLT_JOB_TYP,int& nCnt )
{
	...
	strSql += CRLF + _T("   AND CAST(BANK AS INT) = CAST(") + CLib::Quot(strBANK) + _T(" AS INT)");	// [LGLS 2026-07-19] zero-pad tolerant
	strSql += CRLF + _T("   AND CAST(BAY AS INT) = CAST(") + CLib::Quot(strBAY) + _T(" AS INT)");	// [LGLS 2026-07-19]
	strSql += CRLF + _T("   AND CAST(LEV AS INT) = CAST(") + CLib::Quot(strLEV) + _T(" AS INT)");	// [LGLS 2026-07-19]
