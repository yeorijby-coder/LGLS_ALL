...
BEGIN_MESSAGE_MAP(CLogEqpJobHisSkinDlg, CSkinDialog)
	ON_MESSAGE(SSM_CLICK, &CLogEqpJobHisSkinDlg::OnLglsHdrSort)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_SIZE()
	ON_WM_CLOSE()
...
// [LGLS] 그리드 헤더 클릭 정렬
LRESULT CLogEqpJobHisSkinDlg::OnLglsHdrSort(WPARAM wParam, LPARAM lParam)
{
...
void CLogEqpJobHisSkinDlg::RenameResource( EN_LANG enLang)
{
	...
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_FROM_LOCATION, strValue);
	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobhislog\\"), _T("dlg_jobhislog"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("tolocation"), (int)enLang);
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_LBL_TO_LOCATION, strValue);
	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobhislog\\"), _T("dlg_jobhislog"), strExtension);
