...
void CRtv::AutoRunProc()
{
	...
	CSingleLock _lockRtv(&m_pDoc->m_csEqpData, TRUE);
...
CString CRtv::GetSelectQry() //kdh20190521
{
	...
	strSql.Format(_T(" SELECT RD.RTV_NO,										  \n")
		// [LGLS] LGLS_MCS_IO rtv_data 실컬럼 -> 기존 표시 별칭 매핑
		//  (원본 ECS Vehicle 변수 정렬 컬럼 사용: subsystem_status/unload_complete/pallet_on_vehicle 등)
		_T("        CASE WHEN RD.SUBSYSTEM_STATUS_RD = '2' THEN '1' ELSE '0' END AS ACTIVE_MODE_RD,  \n")
		_T("        ") + m_pDoc->NVL + _T("(RD.AUTO_MODE_RD,'0') AS AUTO_MODE_RD, \n")
		_T("		") + m_pDoc->NVL + _T("(RD.UNLOAD_COMPLETE_RD,'0') AS COMPLETE_RD,  \n")
		_T("        ") + m_pDoc->NVL + _T("(RD.SENSOR_RTV_RD,'0') AS SENSOR_FK_RD, \n")
		_T("        ") + m_pDoc->NVL + _T("(RD.SUBSYSTEM_STATUS_RD,'0') AS UCSTATUS_RD,  \n")
		_T("        '1' AS USE_FORK_RD, \n")
		_T("        ") + m_pDoc->NVL + _T("(RD.POS_H_RD,'0') AS POS_H_RD,  \n")
		_T("        ") + m_pDoc->NVL + _T("(RD.TO_01_OD,'0') AS ARRIVEHS_NO_FK1_RD,  \n")
		_T("        '0' AS ARRIVEHS_NO_FK2_RD, \n")
		_T("        ") + m_pDoc->NVL + _T("(RD.FROM_01_OD,'0') AS DEPARTHS_NO_FK1_RD,  \n")
		_T("        '0' AS DEPARTHS_NO_FK2_RD, \n")
		_T("        ") + m_pDoc->NVL + _T("(RD.ERR_CODE_RD,'0') AS ERR_CODE_RD,  \n")
		_T("        ") + m_pDoc->NVL + _T("(RD.JOB_TYP_OD,'0') AS JOB_TYP_RD, \n")
		_T("        '0' AS FORKPOS_FK1_RD,  \n")
		_T("        '0' AS FORKPOS_FK2_RD, \n")
		_T("        ") + m_pDoc->NVL + _T("(RD.PALLET_ON_VEHICLE_RD,'0') AS LUGG_NO_FK1_RD,  \n")
		_T("        '0' AS LUGG_NO_FK2_RD, \n")
