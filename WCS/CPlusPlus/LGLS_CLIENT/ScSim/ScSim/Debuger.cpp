...
void CDebuger::Assert(BOOL bValid, LPCTSTR lpszLog, int nLine, LPCSTR lpszFile)
{
	...
	CString strLog;
	if (lpszLog == NULL)
		strLog.Format(_T("[%s][%s][%d] ASSERT(FALSE)! \r\n"), COleDateTime::GetCurrentTime().Format(_T("%Y-%m-%d %H:%M:%S")), lpszFile, nLine);
	else
		strLog.Format(_T("[%s][%s][%d] ASSERT(FALSE)! %s \r\n"), COleDateTime::GetCurrentTime().Format(_T("%Y-%m-%d %H:%M:%S")), lpszFile, nLine, lpszLog);
...
void CDebuger::Assert(int nIndex, int nSize, LPCTSTR lpszLog, int nLine, LPCSTR lpszFile)
{
	...
	CString strLog;
	if (lpszLog == NULL)
		strLog.Format(_T("[%s][%s][%d] ASSERT(Invalid Array Index)! (INDEX=%d, SIZE=%d) \r\n"), COleDateTime::GetCurrentTime().Format(_T("%Y-%m-%d %H:%M:%S")), lpszFile, nLine, nIndex, nSize);
	else
		strLog.Format(_T("[%s][%s][%d] ASSERT(Invalid Array Index)! (INDEX=%d, SIZE=%d) %s \r\n"), COleDateTime::GetCurrentTime().Format(_T("%Y-%m-%d %H:%M:%S")), lpszFile, nLine, nIndex, nSize, lpszLog);
	WriteLog(strLog, _T("ASSERT.LOG"));	
}
...
{
	CString strLog;
//	strLog.Format(_T("[%s][%s][%d][%s] %s \r\n"), COleDateTime::GetCurrentTime().Format(_T("%Y-%m-%d %H:%M:%S")), lpszFile, nLine, lpszPos, lpszLog);
	strLog.Format(_T("[%s][%d][%s] %s \r\n"), COleDateTime::GetCurrentTime().Format(_T("%H:%M:%S")), ::GetCurrentThreadId(), lpszPos, lpszLog);
	...
	CString strFileName;
	strFileName.Format(_T("%sTRACE.LOG"), COleDateTime::GetCurrentTime().Format(_T("%Y%m%d")));
