...
CJobDlg::CJobDlg(CEcsDoc* pDoc, CTrackInfo* pTrack, CWnd* pParent )
...
BOOL CJobDlg::OnInitDialog() 
{
	...
	int		fmt				= LVCFMT_LEFT;
	int		cx[size]		= {300, 80};
	LPTSTR	pszText[size]	= {_T("목적지"), _T("다음트랙")};
...
void CJobDlg::UpdateList()
{
	...
	//	strStation.Format(_T("[%05d] %s"), m_pTrack->m_nStationArray[k], pStation->m_strName);
...
void CJobDlg::OnBtnAddOfflineJob() 
{
	...
	for (int i=0; i<nLen; i++)
	{
		...
		if (m_pTrack->m_nStationArray[i] == nDestPos)
		{
			m_pTrack->m_nNextTrArray[i] = nNextTr;
			m_pTrack->m_nNextPlcArray[i] = m_pDoc->m_nNextPlcNum;
		}
		else if(pStation != NULL && _ttoi(pStation->m_strID) == nDestPos)
		{
			m_pTrack->m_nNextTrArray[i] = nNextTr;
			m_pTrack->m_nNextPlcArray[i] = m_pDoc->m_nNextPlcNum;
...
void CJobDlg::OnBtnRetry() 
{
	...
	for (int i=0; i<nLen; i++)
	{
		m_pTrack->m_nNextTrArray[i] = 0;
		m_pTrack->m_nNextPlcArray[i] = 0;
	}
