...
void CEcsControl::UpdateTrack(int nPlcNo, int nDeviceNo) 
{
	...
	*/
	...
	//if(nPlcNo > 9)
		//nTrackNum = (nPlcNo%10*10) + nTrackNum;
	...
	if (pTrack != NULL)
	{
		...
		if (pTrack->m_nKind == 0)
		{
			...
			strTrNum.Format(_T("%05d"),pTrack->m_nHSTrack);
...
void CEcsControl::Refresh()
{
	...
	{                                              
			...
			m_pDoc->m_pJobDlg->m_bCenterNext == FALSE && m_pDoc->m_pJobDlg->m_bCenterDest == FALSE && m_pDoc->m_pJobDlg->m_bCrossTrack == FALSE && m_pDoc->m_pJobDlg->m_bCrossDest == FALSE)
		{
			...
			//					UpdateList(nCurrPlcNum+1, (nTrackNum/10)*10);
			//					UpdateBits(nCurrPlcNum+1, (nTrackNum/10)*10);
			//					UpdateTrack(nCurrPlcNum+1, nTrackNum);
			//					nTrackNum = (nTrackNum % 100) * 10;
			UpdateList(m_nCurrPlcNum, (m_nTrackNum%1000)*10);
			UpdateBits(m_nCurrPlcNum, (m_nTrackNum%1000)*10);
			UpdateTrack(m_nCurrPlcNum, m_nTrackNum);
			//pDoc->m_oCvDlg.DoModal();3.
		}
		else if (m_bNextNor == TRUE)
		{
			...
			//m_pDoc->m_nNextPlcNum=_ttoi(strCID.Mid(5, 1));
...
void CEcsControl::OnButton2() 
{
	// TODO: Add your control notification handler code here
	
}

void CEcsControl::OnButton3() 
{
	// TODO: Add your control notification handler code here
	
}

void CEcsControl::OnButton4() 
{
	// TODO: Add your control notification handler code here
...
//	if (strDeviceNo.IsEmpty()) return;
	...
	if (strHexa < _T("0000") || strHexa > _T("FFFF"))	return;
...
//	m_pDoc->m_arrRegData[nPlcNo-1][nDeviceNo+4] = _ttoi(strRgv);
...
//		if(nPlcNo > 9)
//			nDeviceNo = (pTrack->m_nNumber%10-pCv->m_nStTrNum+1)*10;
		m_pDoc->m_arrRegData[nPlcNo-1][nDeviceNo+7] = m_pDoc->m_arrRegData[nPlcNo-1][nDeviceNo+7] | enBit11;
	}
...
//		if(nPlcNo > 9)
//			nDeviceNo = (pTrack->m_nNumber%10-pCv->m_nStTrNum+1)*10;
		if ((m_pDoc->m_arrRegData[nPlcNo-1][nDeviceNo+7] & enBit11) == enBit11)
			m_pDoc->m_arrRegData[nPlcNo-1][nDeviceNo+7] = m_pDoc->m_arrRegData[nPlcNo-1][nDeviceNo+7] ^ enBit11;
	}
...
void CEcsControl::OnBtnAllauto() 
{
	...
	for(int j = 0; j < 30; j++)
	{
 		CCv* pCv = (CCv*) m_pDoc->GetEquipment(CEquipment::enCV, j);
		...
		//		nDeviceNo = (pTrack->m_nNumber%10-pCv->m_nStTrNum+1)*10;
			...
			m_pDoc->m_arrRegData[j-1][nDeviceNo+7] = m_pDoc->m_arrRegData[j-1][nDeviceNo+7] | enBit11;
		}
	}
...
void CEcsControl::OnBtnAllSuspend() 
{
	...
	for(int j = 0; j < 40; j++)
	{
		...
		//	if(j > 9)
		//		nDeviceNo = (pTrack->m_nNumber%10-pCv->m_nStTrNum+1)*10;
			if ((m_pDoc->m_arrRegData[j-1][nDeviceNo+7] & enBit11) == enBit11)
				m_pDoc->m_arrRegData[j-1][nDeviceNo+7] = m_pDoc->m_arrRegData[j-1][nDeviceNo+7] ^ enBit11;
		}	
	}
}
...
/*void CEcsControl::UpdateStationInfo(int nWarehouse, CComboBox& cbxStation)
{
