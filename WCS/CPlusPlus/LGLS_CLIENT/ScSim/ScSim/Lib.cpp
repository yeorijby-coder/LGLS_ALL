...
{
	CString strLog="", strClassName = "", strTemp;
...
//	strClassName = pRTClass->m_lpszClassName;
	...
	strLog.Format(_T("%s"), (LPTSTR)szTemp);
	return strLog;
}
...
{
	DEBUGER_ASSERT_VALID(strCID.GetLength() == enLengthControlID);
	return _ttoi(strCID.Mid(5, 2));
}

int CLib::GetEquipment(CString& strCID)
{
	DEBUGER_ASSERT_VALID(strCID.GetLength() == enLengthControlID);
	return _ttoi(strCID.Mid(2, 2));
}

int CLib::GetEquipKind(CString& strCID)
{
	DEBUGER_ASSERT_VALID(strCID.GetLength() == enLengthControlID);
