...
BOOL CScPair::CheckRequest(int nConnNum)
{
	...
	case enScErrorEmptyRetrieve2:
		{
			...
			if (m_pDoc->m_pHostSv)
				m_pDoc->m_pHostSv->ErrorScReport(pJobItem2, enErrorEmptyRetrieve, TRUE);
		}
		break;
	...
	case enScErrorDualStore2:
		{
			...
			if (m_pDoc->m_pHostSv)
				m_pDoc->m_pHostSv->ErrorScReport(pJobItem2, enErrorDualStore, TRUE);
		}
		break;
