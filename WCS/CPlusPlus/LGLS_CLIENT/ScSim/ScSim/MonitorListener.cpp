...
void CMonitorListener::OnAccept(int nErrorCode) 
{
	...
	pMonitorSv->m_tAccept = COleDateTime::GetCurrentTime();
	...
	if (GetClient(pMonitorSv->m_strPeerIP) != NULL)
	{
		...
		m_pDoc->WriteLog(LOG_TYPE_ERROR, LOG_POS_MONITOR, strLog, _T("CMonitorListener::OnAccept"), NULL, NULL, TRUE);
		delete pMonitorSv;
		return;
	}

	// 클라이언트 Accept시 시스템 시간설정 요청 클라이언트로 송신
	CString strXml = _T(""), strSend = _T("");
	strXml.Format(_T("<DATETIME DATA='%04d%02d%02d%02d%02d%02d'/>"), 
					pMonitorSv->m_tAccept.GetYear(), pMonitorSv->m_tAccept.GetMonth(), pMonitorSv->m_tAccept.GetDay(),
					pMonitorSv->m_tAccept.GetHour(), pMonitorSv->m_tAccept.GetMinute(), pMonitorSv->m_tAccept.GetSecond());
	strSend.Format(_T("%c<ECS><SYS><TIME>%s</TIME></SYS></ECS>%c"), 
					CMonitorServer::enSTX, strXml, CMonitorServer::enETX);
	pMonitorSv->SendData(strSend);

	// 클라이언트 Accept시 중앙설비상태통지를 클라이언트로 송신
	pMonitorSv->SendData(m_pDoc->m_strCEquipInfo);
