...
void CInterfaceSk::OnReceive(int nErrorCode) 
{
	...
	if (m_pEquipment->m_enKind != CEquipment::enSC)
	{
		if (!m_bRecvWaiting)
		{
			CString strRecv;
	...
	else if (m_pEquipment->m_enKind == CEquipment::enSC)
	{
...
BOOL CInterfaceSk::RecvWait(int nIndex)
{
	DEBUGER_ASSERT_VALID(m_pEquipment != NULL);

	m_bRecvWaiting = TRUE;

	if (m_pEquipment->m_bThreadOut[nIndex] == FALSE)
	{
