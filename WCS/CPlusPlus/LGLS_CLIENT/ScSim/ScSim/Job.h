...
public:
	...
	BOOL Arrive2(CJobItem* pJobItem, BOOL bManual = FALSE);
	BOOL Arrive3(CJobItem* pJobItem, CStationInfo* pStation, BOOL bManual = FALSE);
...
public:
	...
	CJobItem* FetchStoreJobByCv(CString& strStoStationID, CString& strTrayId);
	CJobItem* FetchRetrieveJobBySc(int nScNum, CString& strDestStationID);
	CJobItem* FetchRackToRackJobBySc(int nScNum, int nExceptLuggNum = 0);
...
//	CJobItem* FetchLgvCompleteJob(int nArriveTrackNum) { return FetchCompleteJob(enJobStatusLgvComplete, nArriveTrackNum); }
	...
	CJobItem*	FetchRetrieveJobByTwinSc(int nScNum, CString& strDestStationID, int nExceptLuggNum);
