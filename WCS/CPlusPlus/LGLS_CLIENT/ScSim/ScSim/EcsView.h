...
//	CTabCtrl	m_tabLayout;
	...
	BOOL   m_bErrFork2;
	BOOL   m_bNextDualSto;
	BOOL   m_bNextEmptyRet;
	BOOL   m_bNextDualSto2;
	BOOL   m_bNextEmptyRet2;
	BOOL   m_bNextOverSize;
	BOOL   m_bNextFallDown;
...
public:
	...
	BOOL m_bJob1[100];
	BOOL m_bJob2[100];
	BOOL m_bFirstComplete[100];
	BOOL m_bOldFirstComplete[100];
	BOOL m_bEmptyRetrieved[100];
	BOOL m_bLoadPairJob[100];
	BOOL m_bUnLoadPairJob[100];
...
//	void UpdateScInfo(); 
	...
	void OnAllRunMode();
	void OnAllScMode();
	CScInfo* GetSimScInfo();
	
	BOOL SerialConnect(CEquipment* pEquipment, int nIndex);

public:
	void CheckRuningJob(int nPlcNum);
	void CheckRequest(int nPlcNum); 
	void CheckJobRun(int nPlcNum, int nFork = 1, int nJobType = 1); 
//	void CheckRetJobRun(int nFork = 1); 
//	void DeleteData(int nFork); 
//	void CheckH2HJobRun(int nFork); 
//	void CheckR2RJobRun(int nFork); 
	void CheckForkJobRun(int nPlcNum, int nFork, int nJobType, int nForkJobType); 
	BOOL CheckCanPairJob(int nPlcNum, int nJobType, BOOL bR2RSto = FALSE);
	BOOL CheckCanPrePairJob(int nPlcNum, int nJobType, int nForkJobType); 
	void CheckOneJobRun(int nPlcNum, int nFork, int nJobType, int nForkJobType = 0); 
	void CheckEmptyRet(int nPlcNum, int nFork);
	void CheckDaulSto(int nPlcNum, int nFork);
...
//	afx_msg void OnButton8();
	afx_msg void OnBtnRunMode(); 
	afx_msg void OnBtnScMode(); 
	afx_msg void OnUnSetAllSuspend(); 
	afx_msg void OnSetAllSuspend();
	afx_msg void OnBtnProd(); 
	afx_msg void OnBtnRunStatus(); 
	afx_msg void OnBtnActive(); 
	afx_msg void OnBtnJobComplete(); 
	afx_msg void OnBtnDualStore(); 
	afx_msg void OnBtnEmptyRetrieve(); 
	afx_msg void OnBtnFork2Pos(); 
	afx_msg void OnBtnFork1Pos(); 
	afx_msg void OnBtnError(); 
	afx_msg void OnBtnVirtical(); 
	afx_msg void OnBtnHorizontal(); 
//	afx_msg void OnTestReStartHostListener();
	...
	DECLARE_MESSAGE_MAP()
public:
	afx_msg void OnBnClickedBtnSerialConnect();
};
