...
{
	CString strHS;
	strHS.Format(_T("[%d] TR%03d"), m_nPos, m_pTrack->m_nNumber);
	return strHS;
}
...
/*
CTrackInfo* CTrackHSArray::FetchByPair(int nTrackNum)
{
	CTrackInfo* pTrackHS = NULL;
	for (int i=0; i<m_nSize; ++i)
	{
		pTrackHS = m_pData[i];
		DEBUGER_ASSERT_VALID(pTrackHS != NULL);
