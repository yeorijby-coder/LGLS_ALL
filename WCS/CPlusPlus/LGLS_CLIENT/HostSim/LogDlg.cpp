...
BOOL CLogDlg::OnInitDialog() 
{
	...
	m_ctlReport.InsertColumn(_T("LOG"), _T("시간"), _T("작업번호"), _T("구분"), _T("제품ID"), _T("LOGGING 내용"), _T("ETX"));
...
void CLogDlg::OnButtonSort() 
{
	...
	if (m_btnSort.isMultiClicked())
	{
		...
		menu.AppendMenu(MF_STRING, LOG_SORT_PRODID, _T("제품ID별 검색"));
...
void CLogDlg::OnLogSaveas() 
{
	...
	CFileDialog FileDlg(FALSE, NULL, _T("*.txt"), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, _T(" ECS Log Files - Text Format (*.txt) | *.txt ||"));
	FileDlg.m_ofn.lpstrInitialDir = m_pDoc->m_pConfig->m_strLogPath;
	FileDlg.m_ofn.lpstrTitle = _T("ECS 작업기록 저장");
	...
	TRY
	{
		...
		LPCTSTR pFileName = strFile;
		pFile = new CFile(pFileName, CFile::modeCreate | CFile::modeWrite);
	}
	CATCH( CFileException, e )
	{
		strTemp.Format(_T("CLogDlg::OnButtonSaveas.. File could not be opened e->m_cause=[%d]"), e->m_cause);
...
// 	if (hdc == NULL)
// 	{
// 		AfxMessageBox("프린터 드라이버가 정상적으로 작동하지 않습니다..! ");
// 		return;
// 	}
// 
...
// 	char Buff[1024];
// 
// 	sprintf(Buff, "Logging file dated %s........", Context.m_strFileName);
