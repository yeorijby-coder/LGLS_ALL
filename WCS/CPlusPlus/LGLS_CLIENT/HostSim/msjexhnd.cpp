...
{
	_tcscpy( m_szLogFileName, pszLogFileName );			// 원본은 이것(줄-Line)만!
}

#define		COPY_BUFFER_SIZE		99999
void MSJExceptionHandler::FileCopy(CString strSourceFileName, CString strTargetFileName)
{
	CString strLog;
	UINT unReadByte;
                                    ...
                                    PEXCEPTION_POINTERS pExceptionInfo )
{
	...
	TCHAR *ptr = _tcstok(m_szLogFileName, seps);
	...
	wsprintf(tcTemp, _T("%s%d%02d%02d_%02d%02d%02d.RPT"), ptr, nYY, nMM, nDD, nHH, nmm, nSS);
	
//	MessageBox(NULL, tcTemp, pszDot, MB_OK);

    m_hReportFile = CreateFile( tcTemp, //m_szLogFileName,		// 원본 
                                GENERIC_WRITE,
                                0,
                                0,
                                OPEN_ALWAYS,
    ...
    if ( m_hReportFile )
    {
		...
		CString strTemp;
		strTemp.Format(_T(".\\Job\\Job_%d%02d%02d_%02d%02d%02d.Ecs"), nYY, nMM, nDD, nHH, nmm, nSS);
		FileCopy(_T(".\\Job\\Job.Ecs"), strTemp);
    }


	{
		CString strAppName = _T("EcsDump");
		...
		if (GetModuleFileName( NULL, (LPWSTR)szDbgHelpPath, _MAX_PATH ))
		{
		...
		if (hDll)
		{
		   MINIDUMPWRITEDUMP pDump = (MINIDUMPWRITEDUMP)::GetProcAddress( hDll, "MiniDumpWriteDump" );
			if (pDump)
			{
				...
				//memcpy(szDumpPath, g_strEcsPath, g_strEcsPath.GetLength()-1);
				...
				CString strDumpPath;// = g_strEcsPath + _T("\\") + m_szAppName + _T(".dmp");
				strDumpPath.Format(_T("%s\\%s_%s.dmp"),g_strEcsPath, strAppName, COleDateTime::GetCurrentTime().Format(_T("%Y%m%d%H%M%S")));


//				// ask the user if they want to save a dump file
//				if (::MessageBox( NULL, _T("Something bad happened in your program, would you like to save a diagnostic file?"), strAppName, MB_YESNO )==IDYES)
//				{
					// create the file
				   HANDLE hFile = ::CreateFile( strDumpPath, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS,
					...
					if (hFile!=INVALID_HANDLE_VALUE)
					{
					   ...
					   BOOL bOK = pDump( GetCurrentProcess(), GetCurrentProcessId(), hFile, MiniDumpNormal, &ExInfo, NULL, NULL );
						if (bOK)
						{
							sprintf( szScratch, "Saved dump file to '%s'", strDumpPath );
							szResult = (LPCTSTR)szScratch;
							//retval = EXCEPTION_EXECUTE_HANDLER;
						}
						else
						{
							sprintf( szScratch, "Failed to save dump file to '%s' (error %d)", strDumpPath, GetLastError() );
							szResult = (LPCTSTR)szScratch;
						}
						::CloseHandle(hFile);
					}
					...
					{
						sprintf( szScratch, "Failed to create dump file '%s' (error %d)", strDumpPath, GetLastError() );
						szResult = (LPCTSTR)szScratch;
					}
//				}
			}
...
//		if (szResult)
//			::MessageBox( NULL, szResult, strAppName, MB_OK );
	}
...
void MSJExceptionHandler::IntelStackWalk( PCONTEXT pContext )
{
	...
	//BOOL Pol :: Clib :: MSJExceptionHandler :: GetLogicalAddress	(	PVOID 	주소 ,
	//																	PTSTR 	szModule ,
	//																	DWORD 	len ,
	//																	DWORD & 	섹션 ,
	//																	DWORD & 	오프셋 
...
void MSJExceptionHandler::ImagehlpStackWalk( PCONTEXT pContext )
{
    ...
    _tprintf( _T("                    Logical addr          \n") );
    _tprintf( _T("Address   Frame     Section:Offset  Module\n") );
    _tprintf( _T("                    SymbolName+SymDisplacement\n") );
