...
void CUserManagerDlg::OnQueryClicked() 
{
	...
	TRY 
	{
		...
		{
			CWnd::MessageBox(_T("조회하고자 하는 내용이 없습니다."), strCap, MB_OK | MB_ICONINFORMATION);
			return;
		}
		...
		while (ar.ReadString(str))
		{
			...
			m_cbxLevel.GetLBText(_ttoi(strArray.GetAt(3)), str);
...
void CUserManagerDlg::OnInsertClicked() 
{
	...
	{
		CWnd::MessageBox(_T("ADMIN으로는 계정을 생성할 수 없습니다."), strCap, MB_OK | MB_ICONWARNING);
		return;
	}

	if (!CheckList()) return;

	if (CWnd::MessageBox(m_strUserID + _T(" 사용자 정보를 입력하시겠습니까?"), strCap, MB_YESNO) != IDYES)
		return;
	...
	str.Format(_T("%04d-%02d-%02d %02d:%02d:%02d"), dt.GetYear(), dt.GetMonth(), dt.GetDay(), dt.GetHour(), dt.GetMinute(), dt.GetSecond());
	m_fpSpread.SetText(++nCol, nRow, variant_t(str));
...
void CUserManagerDlg::OnUpdateClicked() 
{
	...
	{
		CWnd::MessageBox(_T("수정하고자 하는 내용이 없습니다."), strCap, MB_OK | MB_ICONINFORMATION);
		return;
	}
	...
	{
		CWnd::MessageBox(_T("수정하고자 하는 사용자 ID는 동일해야 합니다."), strCap, MB_OK | MB_ICONWARNING);
	...
	if (AfxMessageBox(m_strUserID + _T(" 사용자 정보를 수정하시겠습니까?"), MB_YESNO) != IDYES)
		return;
...
void CUserManagerDlg::OnDeleteClicked() 
{
	...
	if (m_fpSpread.GetMaxRows() == 0)
	{
		CWnd::MessageBox(_T("삭제하고자 하는 내용이 없습니다."), strCap, MB_OK | MB_ICONINFORMATION);
		return;
	}
	...
	{
		CWnd::MessageBox(_T("삭제하고자 하는 사용자를 선택하십시오."), strCap, MB_OK | MB_ICONINFORMATION);
		return;
	}

	if (m_strUserID == _T("ADMIN"))
	{
		CWnd::MessageBox(_T("ADMIN 계정은 삭제할 수 없습니다."), strCap, MB_OK | MB_ICONWARNING);
		return;
	}

	if (CWnd::MessageBox(m_strUserID + _T(" 사용자 정보를 삭제하시겠습니까?"), strCap, MB_YESNO) != IDYES)
		return;
...
void CUserManagerDlg::OnExcelClicked() 
{
	...
	if (m_fpSpread.GetMaxRows() == 0)
	{
		CWnd::MessageBox(_T("저장하고자 하는 내용이 없습니다."), strCap, MB_OK | MB_ICONINFORMATION);
		return;
	}
...
//	m_fpSpread.ExportToExcel(strPathName, _T("Sheet1"), "");
	m_fpSpread.ExportExcelBook(strPathName, _T(""));
...
BOOL CUserManagerDlg::CheckList()
{
	...
	{
		CWnd::MessageBox(_T("사용자 ID를 입력해주십시오."), strCap, MB_OK | MB_ICONINFORMATION);
		(GetDlgItem(IDC_USER_ID))->SetFocus();
		return FALSE;
	}
	...
	{
		CWnd::MessageBox(_T("사용자 이름을 입력해주십시오."), strCap, MB_OK | MB_ICONINFORMATION);
	...
	{
		CWnd::MessageBox(_T("비밀번호를 입력해주십시오."), strCap, MB_OK | MB_ICONINFORMATION);
	...
	{
		CWnd::MessageBox(_T("비밀번호가 일치하지 않습니다. 재입력해 주십시오."), strCap, MB_OK | MB_ICONINFORMATION);
	...
	{
		CWnd::MessageBox(_T("사용자 권한을 선택해 주십시오."), strCap, MB_OK | MB_ICONINFORMATION);
...
//	if (m_fpSpread.GetMaxRows() == 0)	return TRUE;
	...
	{
		CWnd::MessageBox(_T("백업파일 삭제 실패! 재시도해 주십시오."), strCap, MB_OK | MB_ICONERROR);
...
//		TRACE(str);
	...
	CATCH (CFileException, e)
	{
		CWnd::MessageBox(_T("데이터 파일 백업 실패! 재시도해 주십시오."), strCap, MB_OK | MB_ICONERROR);
...
//				pDoc->m_pMonitorListener->SendToClients(strSend);
	...
	CATCH (CException, e)
	{
		CWnd::MessageBox(_T("파일 저장 실패! 재시도해 주십시오."), strCap, MB_OK | MB_ICONERROR);
//		CWnd::MessageBox(CLib::GetExceptionString(e), strCap, MB_OK | MB_ICONERROR);
	}
	END_CATCH
