...
//		#include "C:\\Program Files (x86)\\Microsoft SDKs\\Windows\\v7.0A\\Include\\dbghelp.h"
//	#else
...
void MSJExceptionHandler::FileCopy(PTSTR strSourceFileName, PTSTR strTargetFileName)
{
/*
	PTSTR strLog;
	UINT unReadByte;
                                    ...
                                    PEXCEPTION_POINTERS pExceptionInfo )
{
	...
	wsprintf(tcTemp, "%s%d%02d%02d_%02d%02d%02d.RPT", ptr, nYY, nMM, nDD, nHH, nmm, nSS);
    ...
    if ( m_hReportFile )
    {
		...
		wsprintf(strBackupFile, _T(".\\Job\\Job_%d%02d%02d_%02d%02d%02d.Ecs"), nYY, nMM, nDD, nHH, nmm, nSS);

		CHAR strJobFile[MAX_PATH] = _T(".\\Job\\JOB.ECS");
//		CFile::Rename(strJobFile, strBackupFile);					// 이부분을 할수있는 방법이 없음! 
    }
	
	{
		TCHAR strAppName[MAX_PATH];
		wsprintf(strAppName, "%s", _T("EcsDump"));
...
//				strDumpPath.Format(_T("%s\\%s_%s.dmp"),g_strEcsPath, strAppName, COleDateTime::GetCurrentTime().Format(_T("%Y%m%d%H%M%S")));
				...
				CHAR strDumpPath[MAX_PATH];
//				wsprintf(strDumpPath, _T("%s\\%s.dmp"), ptr, strAppName);
				wsprintf(strDumpPath, _T(".\\%s.dmp"), strAppName);
				   ...
				   HANDLE hFile = ::CreateFile( strDumpPath, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS,
					...
					if (hFile!=INVALID_HANDLE_VALUE)
					{
					   ...
					   BOOL bOK = pDump( GetCurrentProcess(), GetCurrentProcessId(), hFile, MiniDumpNormal, &ExInfo, NULL, NULL );
						if (bOK)
						{
							wsprintf( szScratch, _T("Saved dump file to '%s'"), strDumpPath );
							szResult = szScratch;
