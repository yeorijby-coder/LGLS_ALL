...
void CConfigSub1Page::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_BTN_SC_STO_ERROR, m_btnScStoError);
	DDX_Control(pDX, IDC_BTN_SC_RET_ERROR, m_btnScRetError);
...
BOOL CConfigSub1Page::OnInitDialog() 
{
	...
	m_btnScStoError.m_clrCurrent= m_pConfig->m_clrScStoError;
	m_btnScRetError.m_clrCurrent= m_pConfig->m_clrScRetError;
...
void CConfigSub1Page::OnButtonJobPath() 
{
	...
	if (lpID != NULL)
	{
		if ( SHGetPathFromIDList(lpID, (LPWSTR)szFullPathName) )
			SetDlgItemText(IDC_EDIT_JOB_PATH, (LPCTSTR)szFullPathName);
	}
}
...
void CConfigSub1Page::OnButtonLogPath() 
{
	...
	{
		if ( SHGetPathFromIDList(lpID, (LPWSTR)szFullPathName) )
			SetDlgItemText(IDC_EDIT_LOG_PATH, (LPCTSTR)szFullPathName);
	}	
}
...
void CConfigSub1Page::OnButtonStcPath() 
{
	...
	{
		if ( SHGetPathFromIDList(lpID, (LPWSTR)szFullPathName) )
			SetDlgItemText(IDC_EDIT_STC_PATH, (LPCTSTR)szFullPathName);
	}	
}
...
void CConfigSub1Page::OnButtonInfoPath() 
{
	...
	{
		if ( SHGetPathFromIDList(lpID, (LPWSTR)szFullPathName) )
			SetDlgItemText(IDC_EDIT_INFO_PATH, (LPCTSTR)szFullPathName);
	}	
}
...
void CConfigSub1Page::OnButtonSave() 
{
	...
	m_pConfig->m_clrScStoError	= m_btnScStoError.m_clrCurrent;
	m_pConfig->m_clrScRetError	= m_btnScRetError.m_clrCurrent;
