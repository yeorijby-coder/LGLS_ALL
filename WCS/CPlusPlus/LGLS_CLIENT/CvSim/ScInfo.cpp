...
void CScInfo::ReleaseReport(BOOL bManual)
{
	CEcsDoc* pDoc = m_pEquipment->m_pDoc;
	DEBUGER_ASSERT_VALID(pDoc != NULL);
	...
	if (m_ScInfo[0].m_strDepartInfo.GetLength() >= 6)
		strLocation = m_ScInfo[0].m_strDepartInfo;
	else if (m_ScInfo[0].m_strArriveInfo.GetLength() >= 6)
		strLocation = m_ScInfo[0].m_strArriveInfo;
...
void CScInfo::SetFireAlarm(BYTE ucFireAlarm)
{
	if (m_ScInfo[0].m_ucFireAlarm == ucFireAlarm)
		return;

	m_ScInfo[0].m_ucFireAlarm = ucFireAlarm;
	m_bModified = TRUE;
}
/*
...
CString CScInfo::GetProdLoad()
{
	...
	case enProdExistFork1:	
		...
		else				return _T("Frok #2 화물유");
	case enProdExistFork2:	return _T("Frok #1 화물유");
...
{
	CString strXml;
	strXml.Format(_T("<INFO RCS='%d' RCM='%d' SCM='%d' SCS='%d' HPOS='%d' VPOS='%d' PSTS='%d' FPOS='%d' FSTS='%d' ERR='%d' LNO='%d' LNO2='%d'JT='%d' DI='%s' DI2='%s' AI='%s' AI2='%s'JSTS='%d' INVK='%d' INVK2='%d' ILNO='%d' ILNO2='%d' IJT='%d' IJT2='%d' STOS='%d' RETS='%d'/>"), 
		m_ucRcStatus, m_ucRcMode, m_ucScMode, m_ucScStatus, m_wHorizontalPos, m_wVerticalPos, m_ucProdLoad, m_ucForkPos, m_ucForkStatus,
...
void CScInfo::InvokeControl()
{
	...
	m_pControl->m_nForkPos = (m_wHorizontalPos >= 254) ? 1 : m_wHorizontalPos;
	if( m_pControl2 )
		m_pControl2->m_nForkPos = (m_wHorizontalPos >= 254) ? 1 : m_wHorizontalPos;
	if( m_pControl3 )
		m_pControl3->m_nForkPos = (m_wHorizontalPos >= 254) ? 1 : m_wHorizontalPos;
	...
	{
		m_pControl->m_clrFork = GetForkColor2();		// JBY 0829
		m_pControl->m_clrFork2 = GetForkColor1();		// JBY 0829

		if( m_pControl2 )
		{
			m_pControl2->m_clrFork = GetForkColor2();		// JBY 0829
			m_pControl2->m_clrFork2 = GetForkColor1();		// JBY 0829
		}

		if(  m_pControl3 )
		{
			m_pControl3->m_clrFork = GetForkColor2();		// JBY 0829
			m_pControl3->m_clrFork2 = GetForkColor1();		// JBY 0829
		}
