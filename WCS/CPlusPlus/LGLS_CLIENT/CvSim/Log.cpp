...
void CLog::Write(int nType, int nLogPos, int nLuggNum, LPCTSTR lpszContent, LPCTSTR lpszProdID/* =NULL */, BYTE bAlarm/* =0 */, int nErrCode/* =0 */)
{
	...
	COleDateTimeSpan theTimeSpan(1, 0, 0, 0);
...
//		CFile f(strLogFile, CFile::modeCreate | CFile::modeNoTruncate | CFile::modeWrite);
		CStdioFile f((LPCTSTR)strLogFile, CFile::modeCreate | CFile::modeNoTruncate | CFile::modeWrite);
		f.SeekToEnd();
...
void CLog::PumpupAlarm(CLogEvent* pLogEvent)
{
	TRY
	{
		...
		for (POSITION pos=m_listAlarm.GetHeadPosition(); pos!=NULL; )
		{
			pLog = (CLogEvent*)m_listAlarm.GetNext(pos);

			if (pLog == NULL)	continue;

			if (pLogEvent->m_strProdID == pLog->m_strProdID)
			{
				delete pLogEvent;
				return;
			}
...
{
	CString strLogEvent;
	strLogEvent.Format(_T("%14s %02d %02d %05d %-13s %s\r\n"),
...
void CLogEvent::SetString(CString& strLogEvent)
{
	...
	m_tTime = COleDateTime(nYear, nMonth, nDay, _ttoi(strLogEvent.Mid(8,2)), _ttoi(strLogEvent.Mid(10,2)), _ttoi(strLogEvent.Mid(12,2)));
	m_nType = _ttoi(strLogEvent.Mid(15,2));
	m_nPos = _ttoi(strLogEvent.Mid(18,2));
	m_nLuggNum = _ttoi(strLogEvent.Mid(21,5));
	m_strProdID = strLogEvent.Mid(27,13);
...
CString CLogEvent::GetPosition(int nPos)
{
	...
	case	LOG_POS_RTV:		return CString("RTV");
	...
	case	LOG_POS_THS:		return CString("THS");
...
{
	CString strTemp;
	strTemp.Format(_T("발생일시 : %s\\작업번호 : %s\\TRAY ID  : %s\\구분         : %s"),
