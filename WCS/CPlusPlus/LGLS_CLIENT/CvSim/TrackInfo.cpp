...
void CTrackInfo::Initialize()
{
	...
	//m_sSeparatelyETC.m_bUseSeparatelyETC = FALSE;
	//m_sSeparatelyETC.m_bAuto = FALSE;
	//m_sSeparatelyETC.m_bEmergency = FALSE;
	//m_sSeparatelyETC.m_bProductSensor = FALSE;
	//m_sSeparatelyETC.m_bStoStation = FALSE;
	//m_sSeparatelyETC.m_bRetStation = FALSE;
	//m_sSeparatelyETC.m_bScStoHS = FALSE;
	//m_sSeparatelyETC.m_bScRetHS = FALSE;
	//m_sSeparatelyETC.m_bRgvDepartHS = FALSE;
	//m_sSeparatelyETC.m_bRgvArriveHS = FALSE;

	m_nEtcCount = m_pEquipment->m_pDoc->m_strEtcKeyWords.GetCount();
	m_sSeparatelyETC.SetSize(m_nEtcCount);
	for (int i = 0; i < m_nEtcCount; i++)
	{
		m_sSeparatelyETC[i] = FALSE;
	}
}
...
void CTrackInfo::SetTurn(int nTurn)
{
	if (m_nTurn == nTurn)
		return;
...
//	strTrack.Format(_T("TRACK#%03d"), m_nNumber + ((m_nCvPlcNum ==0) ? 1*1000 : m_nCvPlcNum*1000));
	//strTrack.Format(_T("TRACK#%05d"), m_nNumber); - 기존
	strTrack.Format(_T("TRACK#%03d"), m_nNumber); //G동
	return strTrack;
}
...
BOOL CTrackInfo::GetMapCVValue(WORD wStatus)
{
	CObject* pStatusValue = NULL;
	if (m_mapStatusInfos.Lookup(wStatus, pStatusValue) == FALSE)
		return FALSE;
...
COLORREF CTrackInfo::GetCvColor()
{
	...
	int nJobType = m_nJobType;
	if (m_pEquipment->m_pDoc->m_pDeviceMaps[m_nCvPlcNum - 1]->m_bUseSeparatelyETC == TRUE)
	{
		//int nSelIndex = m_pEquipment->m_pDoc->GetSignalIndex("Auto");

		//if (nSelIndex != -1)
		//{
		//	if (m_sSeparatelyETC[nSelIndex] == FALSE)
		//		return DARK_GRAY;
		//}

		int nDeviceNo = (m_nNumber - m_pEquipment->m_nStTrNum + 1) * m_pEquipment->m_pDoc->m_nWordCnt;
		int nAuto = m_pEquipment->m_pDoc->GetAddrByName(m_nCvPlcNum - 1, nDeviceNo, _T("Auto"));

		nJobType = m_pEquipment->m_pDoc->GetAddrByName(m_nCvPlcNum - 1, nDeviceNo, _T("JobType"));
	...
	if (nJobType == enJobTypeNone && IsProductSensing())
		return WHEAT;
...
void CTrackInfo::InvokeControl(CDciTrackCtrl*	pTrackCtrl)
{
	...
	int nSelectedTab = 0;
	if (pView != NULL && pView->m_tabLayout.GetSafeHwnd())
	{
		nSelectedTab = pView->m_tabLayout.GetCurSel() + 1;
	}

	if (m_nTabNum != nSelectedTab)
		return;

	int nTrackNum = _ttoi(pTrackCtrl->m_strCID.Right(3));
	int nSelIndex = 0;
	int nDeviceNo = -1;	// nTrackNum * 2;
	...
	{
		// 일단 하드코딩
		int nTrackNo = nTrackNum % 100;
		nDeviceNo = nTrackNo * m_pEquipment->m_pDoc->m_nWordCnt;

		//m_pEquipment->m_pDoc->m_bParent = TRUE;
	...
	for (int i=0; i<pTrackCtrl->GetItemSize(); ++i)
	{
		...
		WORD wStatusData = GetStatusData(wStatus);
		//WORD wLGVArriveData = GetLGVArriveData(0);
		switch (wStatus)
		{
		case enStatusStoSTReady:
			if (m_pEquipment->m_pDoc->m_pDeviceMaps[m_nCvPlcNum - 1]->m_bUseSeparatelyETC == TRUE)
			{
				//nDeviceNo = nTrackNum * 2;
				bTemp = m_pEquipment->m_pDoc->GetAddrByName(m_nCvPlcNum - 1, nDeviceNo, _T("StoStation"));
				//pTrackCtrl->m_items[i].m_clrItem = m_bSuspend ? pConfig->m_clrSuspend : (bTemp ? pConfig->m_clrStoST : clrStatusNone);
				pTrackCtrl->m_items[i].m_clrItem = bTemp ? pConfig->m_clrStoST : clrStatusNone;
			}
			else
			{
				//pTrackCtrl->m_items[i].m_clrItem = m_bSuspend ? pConfig->m_clrSuspend :  (wStatusData ? pConfig->m_clrStoST : clrStatusNone);
				pTrackCtrl->m_items[i].m_clrItem = wStatusData ? pConfig->m_clrStoST : clrStatusNone;
			}
			break;
		...
		case enStatusRetSTReady:
			...
			{
				//nDeviceNo = nTrackNum * 2;
				bTemp = m_pEquipment->m_pDoc->GetAddrByName(m_nCvPlcNum - 1, nDeviceNo, _T("RetStation"));
				//pTrackCtrl->m_items[i].m_clrItem = m_bSuspend ? pConfig->m_clrSuspend :  (bTemp ? pConfig->m_clrRetST : clrStatusNone);
				pTrackCtrl->m_items[i].m_clrItem = bTemp ? pConfig->m_clrRetST : clrStatusNone;
			}
			else
			{
				//pTrackCtrl->m_items[i].m_clrItem = m_bSuspend ? pConfig->m_clrSuspend :  (wStatusData ? pConfig->m_clrRetST : clrStatusNone);
				pTrackCtrl->m_items[i].m_clrItem = wStatusData ? pConfig->m_clrRetST : clrStatusNone;
			}
			break;
		...
		case enStatusArvSTReady:
			if (m_pEquipment->m_pDoc->m_pDeviceMaps[m_nCvPlcNum - 1]->m_bUseSeparatelyETC == TRUE)
			{
				//nDeviceNo = nTrackNum * 2;
				bTemp = m_pEquipment->m_pDoc->GetAddrByName(m_nCvPlcNum - 1, nDeviceNo, _T("RetStation"));
				//pTrackCtrl->m_items[i].m_clrItem = m_bSuspend ? pConfig->m_clrSuspend :  (bTemp ? pConfig->m_clrArvST : clrStatusNone);
				pTrackCtrl->m_items[i].m_clrItem = bTemp ? pConfig->m_clrArvST : clrStatusNone;
			}
			else
			{
				//pTrackCtrl->m_items[i].m_clrItem = m_bSuspend ? pConfig->m_clrSuspend :  (wStatusData ? pConfig->m_clrArvST : clrStatusNone);
				pTrackCtrl->m_items[i].m_clrItem = wStatusData ? pConfig->m_clrArvST : clrStatusNone;
			}
			break;
		...
		case enStatusLfDepartHSReady:
			...
			{
				//nDeviceNo = nTrackNum * 2;
				bTemp1 = m_pEquipment->m_pDoc->GetAddrByName(m_nCvPlcNum - 1, nDeviceNo, _T("ScStoHS"));
				bTemp2 = m_pEquipment->m_pDoc->GetAddrByName(m_nCvPlcNum - 1, nDeviceNo, _T("RtvDepartHS"));
				//pTrackCtrl->m_items[i].m_clrItem = m_bSuspend ? pConfig->m_clrSuspend :  ((bTemp1 || bTemp2) ? pConfig->m_clrDepart : clrStatusNone);
				pTrackCtrl->m_items[i].m_clrItem = (bTemp1 || bTemp2) ? pConfig->m_clrDepart : clrStatusNone;
			}
			else
			{
				//pTrackCtrl->m_items[i].m_clrItem = m_bSuspend ? pConfig->m_clrSuspend :  (wStatusData ? pConfig->m_clrDepart : clrStatusNone);
				pTrackCtrl->m_items[i].m_clrItem = wStatusData ? pConfig->m_clrDepart : clrStatusNone;
			}
			break;
		...
		case enStatusFullReady:
			...
			{
				//nDeviceNo = nTrackNum * 2;
				bTemp1 = m_pEquipment->m_pDoc->GetAddrByName(m_nCvPlcNum - 1, nDeviceNo, _T("ScRetHS"));
				bTemp2 = m_pEquipment->m_pDoc->GetAddrByName(m_nCvPlcNum - 1, nDeviceNo, _T("RtvArriveHS"));
				//pTrackCtrl->m_items[i].m_clrItem = m_bSuspend ? pConfig->m_clrSuspend :  ((bTemp1 || bTemp2) ? pConfig->m_clrArrive : clrStatusNone);
				pTrackCtrl->m_items[i].m_clrItem = (bTemp1 || bTemp2) ? pConfig->m_clrArrive : clrStatusNone;
			}
			else
			{
				//pTrackCtrl->m_items[i].m_clrItem = m_bSuspend ? pConfig->m_clrSuspend :  (wStatusData ? pConfig->m_clrArrive : clrStatusNone);
				pTrackCtrl->m_items[i].m_clrItem = wStatusData ? pConfig->m_clrArrive : clrStatusNone;
			}
			break;
		default:
			DEBUGER_ASSERT_VALID(FALSE);
...
BOOL CTrackInfo::SaveXML()
{
	...
	try
	{
		...
		if (dom.MoveXPath(_T("/Config/Dests")))
		{
			...
			CString strStation, strNextTrack, strNextPlc, strDestPos;
			...
			for (int i=0; i<nLen; ++i)
			{
				...
				strDestPos.Format(_T("%d"), m_nDestPosArray[i]);
...
void CTrackInfo::SetStation(int nStation, int nNextPlc, int nNextTrack, int nDestPos)
{
