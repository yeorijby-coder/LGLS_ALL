...
CEcsDoc::CEcsDoc()
{
	...
	m_bBcrAutoUpdate1 = FALSE;
	m_bBcrAutoUpdate2 = FALSE;


	for (int i = 0; i < STO_STN_CNT; ++i)
	{
		m_bReportedBCD[i] = FALSE;
	}
	//DB LANG 설정
	ConfigDbLang();
}
...
void CEcsDoc::ConfigDbLang()
{
	...
	SYSDATE = _T("SYSDATE");
	NVL = _T("NVL");
	CURRENT_DATE = _T("to_char(SYSDATE, 'YYYY-MM-DD')");
	CURRENT_TIME = _T("to_char(SYSDATE, 'HH24:MI:SS')");
	CURRENT_DATE_NATURAL = _T("to_char(SYSDATE, 'YYYYMMDD')");
	CURRENT_TIME_NATURAL = _T("to_char(SYSDATE, 'HH24MISS')");
#elif  POSTGRESQL
	SYSDATE = _T("NOW()");
	NVL = _T("COALESCE");
	CURRENT_DATE = _T("to_char(NOW(), 'YYYY-MM-DD')");
	CURRENT_TIME = _T("to_char(NOW(), 'HH24:MI:SS')");
	CURRENT_DATE_NATURAL = _T("to_char(NOW(), 'YYYYMMDD')");
	CURRENT_TIME_NATURAL = _T("to_char(NOW(), 'HH24MISS')");
#endif
}
...
CEcsDoc::~CEcsDoc()
{
	...
	if(m_pViewRackDlg != NULL){ delete m_pViewRackDlg;}
...
void CEcsDoc::OnCommandRangeMainFrameMANUAL(UINT nID)
{
	...
	case ID_MANUAL_JOB:
		{
			...
			{
				if (m_pManualLogin == NULL)
				{
					m_pManualLogin = new CManualLogin(this, m_pDlgUrmDBAccess);
					this->m_pManualLogin->Create(IDD_MANUAL_LOGIN);
					CRect MainRect;
					CRect Rect;
					CRect PosRect;
					::AfxGetApp()->GetMainWnd()->GetWindowRect(&MainRect);   
					this->m_pManualLogin->GetWindowRect(&Rect); 
				
					PosRect.left = ((MainRect.right  - MainRect.left) - Rect.Width())  / 2; 
					PosRect.top  = ((MainRect.bottom - MainRect.top)  - Rect.Height()) / 2; 
					this->m_pManualLogin->SetWindowPos(&m_pManualLogin->wndTop, PosRect.left, PosRect.top, 
						Rect.Width(), Rect.Height(), 
						SWP_SHOWWINDOW);
				
				}
				::SetWindowPos(m_pManualLogin->m_hWnd, HWND_TOPMOST, 0,0,0,0, SWP_NOMOVE | SWP_NOSIZE);
				::SetWindowPos(m_pManualLogin->m_hWnd, HWND_NOTOPMOST, 0,0,0,0, SWP_NOMOVE | SWP_NOSIZE);
				ShowWindow(m_pManualLogin->m_hWnd, SW_SHOWNORMAL);
			}
			else
			{
	...
	case ID_MANUAL_RTV:
		{
			if (!Permission(_T("CManualRtv"), INS_YN))
			{
				AfxMessageBox(GetMsgLangDef(_T("권한이 없습니다")));
				return;
			}
...
void CEcsDoc::OnCommandRangeMainFrameMONITORING(UINT nID)
{
	...
	OnCommandRangeMainFrameNATION(ID_LANGUAGE_KOR);
}
	...
	case ID_SYSTEM_INVENTORY:
		{
			...
			if (!Permission(_T("CViewRackDlg"), SEL_YN))
			{
				AfxMessageBox(GetMsgLangDef(_T("권한이 없습니다")));
				return;
			}

			if (m_pViewRackDlg == NULL)
			{
				m_pViewRackDlg = new CViewRackDlg(this);
				this->m_pViewRackDlg->Create(IDD_VIEW_RACK);
				CRect MainRect;
				CRect Rect;
				CRect PosRect;
				::AfxGetApp()->GetMainWnd()->GetWindowRect(&MainRect);   
				this->m_pViewRackDlg->GetWindowRect(&Rect); 

				PosRect.left = ((MainRect.right  - MainRect.left) - Rect.Width())  / 2; 
				PosRect.top  = ((MainRect.bottom - MainRect.top)  - Rect.Height()) / 2; 
				this->m_pViewRackDlg->SetWindowPos(&m_pViewRackDlg->wndTop, PosRect.left, PosRect.top, 
					Rect.Width(), Rect.Height(), 
					SWP_SHOWWINDOW);

			}
			::SetWindowPos(m_pViewRackDlg->m_hWnd, HWND_TOPMOST, 0,0,0,0, SWP_NOMOVE | SWP_NOSIZE);
			::SetWindowPos(m_pViewRackDlg->m_hWnd, HWND_NOTOPMOST, 0,0,0,0, SWP_NOMOVE | SWP_NOSIZE);
			ShowWindow(m_pViewRackDlg->m_hWnd, SW_SHOWNORMAL);
			break;
		}
...
{
	if(IsPermissionDlg(_T("CViewRackDlg")) == FALSE)
	{
		CString strKey = _T("권한 Permission없음");
		CString aaa = m_pLang->GetLangValue(strKey, m_enLang);
		return;
	...
	CViewRackDlg* pDlg = new CViewRackDlg(this);
	pDlg->Create(IDD_VIEW_RACK);
	pDlg->ShowWindow(SW_SHOW);
}
...
{
	int a = m_pUserInfo.GetCount();

	CPermission* pPermission = new CPermission();

	m_pUserInfo.Lookup(pWID_ID , pPermission);

	switch(pEN_PERM)
	{
	case 0:
		return pPermission->m_ArrPerm[EN_PERM::SEL_YN];
		break;
	case 1:
		return pPermission->m_ArrPerm[EN_PERM::UPD_YN];
		break;
	case 2:
		return pPermission->m_ArrPerm[EN_PERM::DEL_YN];
		break;
	case 3:
		return pPermission->m_ArrPerm[EN_PERM::INS_YN];
		break;
	case 4:
		return pPermission->m_ArrPerm[EN_PERM::PRT_YN];
		break;
	case 5:
		return pPermission->m_ArrPerm[EN_PERM::EXE_YN];
		break;
	case 6:
		return pPermission->m_ArrPerm[EN_PERM::DOWN_YN];
		break;
	case 7:
		return pPermission->m_ArrPerm[EN_PERM::WIN_VIEW_YN];
		break;
	}
	return FALSE;
}
