...
//CString CLib::CheckModifyLocation(CString strLocation)
//{
//    CString strTempLocation = _T("");
//    if (strLocation.GetLength() != enLengthLocation)
//        return _T("");
//
//    return strLocation.Left(2) + _T("-") + strLocation.Mid(2,3) + _T("-") + strLocation.Right(2);
//}
...
int CLib::GetStackerNum(int nWarehouse, CString& strLocation, BOOL bDouble)
{
    ...
    //    break;
    //case CJobItem::enWarehouse2:
    //    nScNum += 10;
    //    break;
    //case CJobItem::enWarehouse3:
    //    nScNum += 30;
    //    break;
    //}
...
BOOL CLib::HsDataSelect(CEcsDoc* pDoc, BOOL bStoHs, CString strSC_NO, CString& strHS_MC_NO, CString& strSENSOR0_DATA_RD)
{
    CString strHS_NO = _T("");
    strHS_MC_NO = _T("");
    ...
    strSql.Format(  _T("  SELECT SHD.HS_MC_NO, CD.SENSOR0_DATA_RD	\n")
                    ...
                    _T("     AND SHD.HS_NO = '%s' 				    \n")
                    _T("     AND SHD.SC_NO = '%s'				    \n"), CLib::GetIniStringWH_TYP(), bStoHs ? _T("02") : _T("01"), strSC_NO);
...
bool CLib::BindCombo_ViewUsageRackDlg(CComboBoxWrapper& cbx, CEcsDoc *pDoc, int nEN_LANG, CString strAGING_TYP)
{
   ...
   strSql.Format(_T("       SELECT *                                                \n")
            _T("           FROM (                                                    \n")     
            _T("                 SELECT BANK, SUBSTR(AGING_TYP,1,3) AS AGING_TYP            \n")
            _T("                   FROM CELL_MST                                     \n")
            _T("                  WHERE WH_TYP = '%s'                                 \n")
            _T("                ) CM                                             \n")
...
bool CLib::BindCombo_SC_HS_DEF_DEST_SPREAD(CComboBoxWrapper& cbx, CEcsDoc *pDoc, int nEN_LANG, CString strSC_NO)
{
   ...
   strSql += CRLF + _T("    AND SC_NO = ( SELECT CCD_CD FROM COMMON_CODE WHERE CDX_CD = 'CELL_SC_NO' AND WH_TYP LIKE '%' || ");
