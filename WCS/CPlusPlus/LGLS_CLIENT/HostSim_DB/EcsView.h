...
public:
	...
	char m_JobOrderMsg[1024];
	char m_AlterLocMsg[1024];
	int m_nJobOrderSize;
	int m_nAlterLocSize;
	
	CTabCtrl m_tabLayout;
	int JobOrder(int nJobType, BOOL bManual = FALSE);
	int AlterLocation(BOOL bManual = FALSE);

	void GenerateStoRet(int nIndex, int nLuggNum);

	BOOL CalcurateLocation(BOOL bSto = TRUE);

	BOOL InsertIF_LUGG_STA(CString pLUGGNO
						 , CString pJOB_KIND
						 , CString pLD_CDN_NO
						 , CString pFROM_CV_NO
						 , CString pTO_CV_NO
	...
	BOOL UpdateIF_LUGG_STA(	  CString pLUGGNO
							, CString pTO_AREA		//= _T("")
	...
	BOOL UpdateIF_MC_STA(CString strMC_TYP, CString strMC_NO, CString strIF_STATUS = _T("Y"), BOOL bPopUpMsgDlg = FALSE);
	BOOL SelectIF_LUGG_STA(BOOL bPopUpMsgDlg = FALSE);
	BOOL DeleteIF_LUGG_STA(CString pLUGGNO, BOOL bPopUpMsgDlg = FALSE);
	BOOL ForCvSelectIF_MC_STA(BOOL bPopUpMsgDlg = FALSE);
	BOOL ForScSelectIF_MC_STA(BOOL bPopUpMsgDlg = FALSE);
	BOOL ForBcrSelectIF_MC_STA(BOOL bPopUpMsgDlg = FALSE);
	afx_msg void OnBnClickedChkBcrAutoUpdate();
	afx_msg void OnBnClickedChkBcrAutoUpdate2();
	BOOL m_bIsBcrRead;
};
