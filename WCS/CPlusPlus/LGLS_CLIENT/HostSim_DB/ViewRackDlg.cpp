...
void CViewRackDlg::OnBnClickedBtnCellRet()
{
	...
	if(AfxMessageBox(m_pDoc->GetMsgLangDef(_T("CELL-출고 작업을 생성하시겠습니까?")), MB_YESNO) == IDYES)
	{
		...
		strSql += CRLF + _T("                     ,MANUAL_SEQ.NEXTVAL ");
...
CString CViewRackDlg::GetQrySelect_Main(BOOL bSearch)
{
	...
	strSql += CRLF + _T("								 AND CDTL.AGING_START_DT >= TO_DATE('") + strdtFrom + _T("', 'MM/DD/YYYY HH24:MI:SS')"); //    + CLib::Quot(_T("TO_DATE('{0}','MM/DD/YYYY HH12:MI:SS')"))
	strSql += CRLF + _T("								 AND CDTL.AGING_END_DT <=  TO_DATE('") + strdtTo + _T("', 'MM/DD/YYYY HH24:MI:SS')");
	strSql += CRLF + _T("                    LEFT OUTER JOIN COMMON_CODE CCD_AGING_TYP ");
	strSql += CRLF + _T("                                 ON CCD_AGING_TYP.WH_TYP LIKE '%' || CM.WH_TYP || '%' ");
	strSql += CRLF + _T("                                AND CCD_AGING_TYP.CDX_CD = 'AGING_TYP' ");
	strSql += CRLF + _T("                                AND CCD_AGING_TYP.CCD_CD = SUBSTR(CM.AGING_TYP,0,3) ");
	strSql += CRLF + _T("                    LEFT OUTER JOIN COMMON_CODE CCD_CELL_STATUS ");
	strSql += CRLF + _T("                                 ON CCD_CELL_STATUS.WH_TYP LIKE '%' || CM.WH_TYP || '%' ");
	...
	strSql += CRLF + _T("                                 ON CCD_CELL_USE_DEF.WH_TYP LIKE '%' || CM.WH_TYP || '%' ");
	...
	strSql += CRLF + _T("                                 ON CCD_FIRE_OD_RQ_YN.WH_TYP LIKE '%' || CM.WH_TYP || '%' ");
...
// 	strSql += CRLF + _T("                                AND CCD_CELL_TYP.CCD_CD = CM.CELL_TYP ");
	...
	strSql += CRLF + _T("                                 ON CCD_WH_TYP.WH_TYP LIKE '%' || CM.WH_TYP || '%' ");
	...
	strSql += CRLF + _T("                                 ON CCD_CELL_SC_NO.WH_TYP LIKE '%' || CM.WH_TYP || '%' ");
