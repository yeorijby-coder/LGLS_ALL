...
public:
	enum {enLengthStn = 3, enLengthLocation = 7, enLengthSKILocation = 10 };
	...
	static int	GetStackerNum(int nWarehouse, CString& strLocation, BOOL bDouble = FALSE);						//&& 추가
...
//	static CString CheckModifyLocation(CString strLocation);
...
public:
	...
	static BOOL HsDataSelect(CEcsDoc* pDoc, BOOL bStoHs, CString strSC_NO, CString& strHS_MC_NO, CString& strSENSOR0_DATA_RD);
