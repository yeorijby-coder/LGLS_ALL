...
void CLogEqpSkinDlg::GetEQPLogSelect()
{
	...
	strSql += CRLF + _T("       AND ( INS_DT >= TO_DATE('") + strdtFrom + _T("', 'MM/DD/YYYY HH24:MI:SS')"); 
	strSql += CRLF + _T("       AND   INS_DT <=  TO_DATE('") + strdtTo + _T("', 'MM/DD/YYYY HH24:MI:SS') )");
	if(strEqp_typ != _T(""))
	{
