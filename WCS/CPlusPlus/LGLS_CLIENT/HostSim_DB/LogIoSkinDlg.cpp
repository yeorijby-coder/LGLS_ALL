...
//	m_SpreadSheet.m_rectWnd = rectTemp;//CRect(0, 0, 0, 0);
	...
	m_SpreadSheet.AddColHead(_T("PULP 단수"), 9);
...
void CLogIoSkinDlg::RenameResource( EN_LANG enLang)
{
	...
	SetDlgItemText(IDC_LBL_REQUEST_ID, strValue);
	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_iolog\\"), _T("dlg_iolog"), strExtension);
...
CString CLogIoSkinDlg::GetQrySelect_Main(int nRowCheck, BOOL bSearch)
{
	...
	CString strTRAY_BOTTOM, strTRAY_TOP, strLUGG_NO, strTRACK_START, strTRACK_DEST, strSTART_LOCATION, strDEST_LOCATION;
	...
	strSql += CRLF + _T(" 	  , ") + m_pDoc->NVL + _T("(CCD_PRODUCT_SIZE.CCD_NM_KOR, HIS.PRODUCT_SIZE) AS PRODUCT_SIZE  ");
	...
	strSql += CRLF + _T("   					                AND CCD_PRODUCT_SIZE.CCD_CD = HIS.PRODUCT_SIZE  ");
	...
	if (strTRAY_TOP != _T(""))
	{
		strSql += CRLF + _T("					 AND HIS.BCR_TOP = '") + strTRAY_TOP + _T("'");
	}
	if (strTRAY_BOTTOM != _T(""))
	{
		strSql += CRLF + _T("					 AND HIS.BCR_BOTTOM = '") + strTRAY_BOTTOM + _T("'");
	}
	if (strLUGG_NO != _T(""))
	{
	...
	strSql += CRLF + _T(" ORDER BY HIS.INS_DT DESC, HIS.LUGG_NO DESC  ");
#if POSTGRESQL
	if (strRowCnt != _T("ALL"))
