	...
	case EN_BtnPaste :
		{
			...
			strSql.Format(_T(" UPDATE CV_DATA					\n")
				...
				_T("	AND PLC_NO = '%02s'			\n")
				_T("    AND MC_NO = '%s'			\n")
				_T("    AND OD_RQ_YN = 'N'			  "), pCopyJob->LUGG_NO, strTO_CV, pCopyJob->JOB_TYP, pCopyJob->PRODUCT_SIZE,
	...
	else
	{
		...
		if(strSTATUS == _T("Q") && nLUGG_NO == 0)
		{
			...
			strSql.Format(_T(" UPDATE HOST_EMPTY_PLT						 \n")
						  ...
						  _T("					   AND STATUS = '%s'		 \n")
						  _T("					 ORDER BY INS_DT DESC		 \n")
						  _T("					 LIMIT 1)					   "), m_pDoc->m_WH_TYP, strKIND, m_pTrackInfo->m_pCV_DATA->V_MC_NO, strSTATUS);
