...
// [LGLS 2026-07-24] extra-text CString ?묎렐 吏곷젹?붿슜 ?꾩뿭 ??r
static CCriticalSection g_csExtraText;

void CDciTrackCtrl::SetExtraTextSafe(LPCTSTR s, COLORREF c)
{
	CSingleLock lk(&g_csExtraText, TRUE);
	m_strExtraText = s;
	m_clrExtraText = c;
}

CString CDciTrackCtrl::GetExtraTextSafe(COLORREF* pColor)
{
	CSingleLock lk(&g_csExtraText, TRUE);
...
{
//	m_bExist = FALSE;
	m_clrExtraText = RGB(0,0,0);   // [LGLS] 보조 텍스트 기본 검정
}
...
//		m_logfont.lfHeight = nSize;//long((-MulDiv(m_nFontSize, GetDeviceCaps(hDC, LOGPIXELSY), 72 )) / m_pDCI->GetScale());
		...
		if (m_bExist)
		{
			// [LGLS] 화물을 눈에 띄게: 트랙 내부를 화물색(앰버)으로 채움 (트랙번호는 이후 상단)
			CRect rcCargo = rcControlS; rcCargo.DeflateRect(2, 2);
			pDC->FillSolidRect(rcCargo, m_clrBgColor);
			pDC->Draw3dRect(rcCargo, m_clrBgColor, m_clrBgColor);
		...
		float nRatio = 25.0;
		float fTemp = float(nRatio / 100.0);
...
//			AfxMessageBox(strTemp22); 
		...
		{
			// [LGLS] 내부 아이템이 없어도 트랙빗호는 항상 표시
			// [LGLS 2026-07-19] 보조 텍스트(SC 출고 잔여 수 등)가 설정되면 트랙번호 대신 지정 색으로 표시
		COLORREF clrEx_; CString strEx_ = GetExtraTextSafe(&clrEx_);
		if (!strEx_.IsEmpty())
		{
			pDC->SetTextColor(clrEx_);
			DrawFontText(pDC, strEx_, NULL, nOldBkMode, nOldFgColor);
		}
		else
			DrawFontText(pDC, strTrDisp, NULL, nOldBkMode, nOldFgColor);
			return;
		}
...
//						pDC->Draw3dRect(rcUnitS, RGB(0,0,0), RGB(255,255,255));
		...
		if (m_bExist && 0) // [LGLS] 아이템 가림 방지: 두번째 화물채움 비활성(아이템 위에 덮지 않음)
		{
			// [LGLS] 화물을 눈에 띄게: 트랙 내부를 화물색(앰버)으로 채움 (트랙번호는 이후 상단)
			CRect rcCargo = rcControlS; rcCargo.DeflateRect(2, 2);
			pDC->FillSolidRect(rcCargo, m_clrBgColor);
