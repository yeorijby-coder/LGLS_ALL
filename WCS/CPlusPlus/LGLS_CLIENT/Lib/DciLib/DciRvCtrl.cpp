...
void CDciRvCtrl::InitControl(CDciMaster* pDCI)
{
	...
	m_clrRail = RGB(0,0,0);
	m_nIncrease = 0;
	m_nRow		= 0;	// [LGLS 2026-09-22] 0 = 종전(짧은 변 기준 정사각형)
	m_nCol		= 0;
	m_nProd		= 0;
//	m_nFontSize = 10;
...
//	properties[i++].SetProperty(CDciProperty::PT_DEC, _T("fontsize"));
	...
	// [LGLS 2026-09-22] 포크 크기(격자 칸) - 0 이면 종전
...
//		m_nFontSize	= CConvert::ToInt(properties[i++].m_strValue);
		...
		m_nRow		= CConvert::ToInt(properties[i++].m_strValue);	// [LGLS 2026-09-22]
...
//		properties[i++].m_strValue.Format(_T("%d"), m_nFontSize);
		...
		properties[i++].m_strValue.Format(_T("%d"), m_nRow);	// [LGLS 2026-09-22]
...
//
//   포크를 만든 직후 ForkRectS 를 통해 불린다. 윙(wing)은 그 뒤에 보정된 포크를 기준으로
//   계산되므로 ★윙도 포크에 비례해 커지고 작아진다★ (사용자 확인 - 원래 그렇게 그려진다).
//
...
// [LGLS 2026-09-22] 포크 논리사각형을 row/col 로 보정하고(참조) 화면좌표로 바꾼다 (사용자 정정).
//   포크를 ★만든 직후★ 부른다. 그래야
//     · 포크 그리기(DrawButton 은 논리 rect 를 쓴다) 가 보정된 크기로 나가고,
//     · 뒤따르는 윙(wing) 계산이 이 반환값(rcForkS/T/D)을 기준으로 하므로 ★윙도 포크에 비례★ 한다.
//   (앞 버전은 윙 계산이 끝난 함수 끝에서 보정해 윙이 따라가지 않았다)
CRect CDciRvCtrl::ForkRectS(CRect& rcForkL)
{
	ApplyForkGridL(rcForkL);
