...
BOOL CDib::AttachMapFile(const char* strPathname, BOOL bShare) // for reading
{
	...
	HANDLE hFile = ::CreateFile(CA2CT(strPathname), GENERIC_WRITE | GENERIC_READ,
...
BOOL CDib::CopyToMapFile(const char* strPathname)
{
	...
	HANDLE hFile = ::CreateFile(CA2CT(strPathname), GENERIC_WRITE | GENERIC_READ, 0, NULL,	CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
void CDib::DisplayPalette(CWnd *pWnd)
{
	if(m_hPalette)
	{
		CDC* pDC = pWnd->GetDC();
		CPalette pal;
		pal.Attach(m_hPalette);

		CRect rc;
		pWnd->GetClientRect(rc);
		int iColor = m_nColorTableEntries;
	...
	if(iColor <= 256)
	{
		for(j = 0, top = 0; j < 16 ; j++, top = bottom)
		{
			bottom = (j+1)*rc.bottom/16+1;

			for(i = 0, left = 0; i < 16 ; i++, left = right)
			{
				right = (i+1)*rc.right/16+1;
	...
	pDC->SelectPalette(pOld, FALSE); // ¹ÝÈ¯
	pal.Detach();
	pWnd->ReleaseDC(pDC);
	} // if
}
