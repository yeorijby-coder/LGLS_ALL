...
int CFooButton::m_nRefs = 0;                               // # CFooButton instances
CMapStringToPtr CFooButton::m_btnGroups;                   // button groups
HINSTANCE CFooButton::m_hMsimg32 = NULL;                   // handle to MSIMG32.dll
MSIMG32callBack CFooButton::m_dllGradientFillFunc = NULL;  // pointer to GradientFill() API
...
CFooButton::CFooButton()
{
	// Set default properties
	setType (CFooButton::pushButton);
	setTextStyle (CFooButton::singleLineCenter);
	setTextColor (GetSysColor (COLOR_BTNTEXT));
	setFocusStyle (CFooButton::noFocus);
	setBitmapId (0);
	...
	m_rgbTopGradient = RGB (255, 255, 255);
	m_rgbBottomGradient = RGB (200, 200, 216);
	m_rgbTopGradientHot = RGB (255, 255, 255);
	m_rgbBottomGradientHot = RGB (220, 220, 220);
...
BEGIN_MESSAGE_MAP(CFooButton, CButton)

	//{{AFX_MSG_MAP(CFooButton)
	ON_WM_ERASEBKGND()
	ON_WM_KILLFOCUS()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
...
void CFooButton::setType(CFooButton::Type type)
{
	...
	switch (type)
	{
    case CFooButton::staticButton:
		m_bStatic = true;
		break;
		
    case CFooButton::checkBox:
    case CFooButton::radio:
    case CFooButton::checkButton:
    case CFooButton::pushButton:
		break;
		
    case CFooButton::pushButtonDropDown:
		m_bDropDown = true;
		break;
		
    case CFooButton::hotCheckButton:
    case CFooButton::hotPushButton:
		m_bHot = true;
		break;
		
    case CFooButton::hotPushButtonDropDown:
		m_bHot = true;
		m_bDropDown = true;
		break;
		
    case CFooButton::pushButtonMulti:
		m_bMulti = true;
		break;
		
    case CFooButton::hotPushButtonMulti:
		m_bHot = true;
		m_bMulti = true;
		break;
		
    case CFooButton::hyperlink:
		m_bHyperlink = true;
		break;
	...
	// Reset checked mode
	if ((m_type != CFooButton::checkButton) &&
		(m_type != CFooButton::hotCheckButton))
		m_bChecked = false;
...
void CFooButton::setTextStyle(CFooButton::Text textStyle)
{
	// Reset text styles
	m_bText = false;
	m_bCenter = false;
	...
	// Set text style
	switch (textStyle) {
    case CFooButton::none:
		break;
		
    case CFooButton::singleLine:
		m_bText = true;
		break;
		
    case CFooButton::singleLineCenter:
		m_bText = true;
		m_bCenter = true;
		break;
		
    case CFooButton::multiLine:
		m_bText = true;
		m_bMultiLine = true;
		break;
...
void CFooButton::setFocusStyle(CFooButton::Focus focusStyle)
{
...
void CFooButton::setBitmapId(int nBitmapId, COLORREF rgbTransparent)
{
...
void CFooButton::check(bool bChecked)
{
	if ((m_type == CFooButton::checkButton) ||
		(m_type == CFooButton::hotCheckButton) ||
		(m_type == CFooButton::checkBox) ||
		(m_type == CFooButton::radio)) {
		m_bChecked = bChecked;
		HWND hWnd = GetSafeHwnd();
		if ((hWnd != NULL) && ::IsWindow (hWnd))
			Invalidate();
	...
	if (isChecked() && !m_strButtonGroup.IsEmpty()) {
		...
		for (int nIndex=0; (nIndex < nButtons); nIndex++) {
			...
			if (hWnd != GetSafeHwnd()) {
				if (IsWindow (hWnd)) {
					CFooButton* pButton = (CFooButton *) CWnd::FromHandle (hWnd);
					ASSERT (pButton != NULL);
...
void CFooButton::clearMultiClick()
{
	if ((m_type == CFooButton::pushButtonMulti) ||
		(m_type == CFooButton::hotPushButtonMulti)) {
		m_bMultiClicked = false;
		HWND hWnd = GetSafeHwnd();
		if ((hWnd != NULL) && ::IsWindow (hWnd))
			Invalidate();
...
int CFooButton::displayPopupMenu(CMenu* pMenu)
{
	Invalidate();
	if (pMenu != NULL) {
		CRect  rectButton;
...
void CFooButton::enable(bool bEnable)
{
...
bool CFooButton::addToGroup(CString strGroupName)
{
...
//! @return   true if successful, false otherwise.
bool CFooButton::removeFromGroup()
{
...
/////////////////////////////////////////////////////////////////////////////
// CFooButton message handlers

void CFooButton::PreSubclassWindow() 
{
...
void CFooButton::OnKillFocus(CWnd* pNewWnd)
{
	HWND hWnd = GetSafeHwnd();
	if ((hWnd != NULL) && ::IsWindow (hWnd))
		Invalidate();
...
void CFooButton::OnLButtonDown (UINT nFlags, CPoint point)
{
	multiHitTest (point);   // sets m_bMultiClicked
	CButton::OnLButtonDown (nFlags, point);
}

void CFooButton::OnLButtonUp (UINT nFlags, CPoint point)
{
		...
		(m_type == CFooButton::radio))
	{
		...
		if (rectButton.PtInRect (point))
			if ((m_type != CFooButton::radio) && m_strButtonGroup.IsEmpty())
				check (!isChecked());
			else
				check (true);
	}
...
void CFooButton::OnMouseMove(UINT nFlags, CPoint point) 
{
...
LRESULT CFooButton::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
...
BOOL CFooButton::OnEraseBkgnd(CDC* pDC) 
{
...
BOOL CFooButton::OnSetCursor (CWnd* /* pWnd */, UINT /* nHitTest */, UINT /* message */)
{
	if (m_bHyperlink)
	{
...
void CFooButton::OnSetFocus(CWnd* pOldWnd)
{
	HWND hWnd = GetSafeHwnd();
	if ((hWnd != NULL) && ::IsWindow (hWnd))
		Invalidate();
...
void CFooButton::DrawItem (LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	CDC* pDC = CDC::FromHandle (lpDrawItemStruct->hDC);
	ASSERT (pDC != NULL);
	
	// Draw frame for non-radio and non-checkbox buttons
	if ((m_type != CFooButton::checkBox) && (m_type != CFooButton::radio))
...
/////////////////////////////////////////////////////////////////////////////
// CFooButton helper functions

//! Draws the button's frame
void CFooButton::drawFrame(CDC* pDC, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	ASSERT (pDC != NULL);
...
void CFooButton::drawRadioCheckFrame(CDC* pDC, LPDRAWITEMSTRUCT lpDrawItemStruct, int& nLeftEdge)
{
	ASSERT (pDC != NULL);
	ASSERT (lpDrawItemStruct != NULL);
	...
	// Draw frame
	UINT uFrameCtrl = (m_type == CFooButton::checkBox ? DFCS_BUTTONCHECK : DFCS_BUTTONRADIO);
	if (m_bChecked)
		uFrameCtrl |= DFCS_CHECKED;
...
void CFooButton::drawBitmap(CDC* pDC, LPDRAWITEMSTRUCT lpDrawItemStruct, int& nBitmapRightEdge)
{
	ASSERT (pDC != NULL);
	ASSERT (lpDrawItemStruct != NULL);
...
//	HBITMAP hBitmap = ::LoadBitmap (AfxGetResourceHandle(), MAKEINTRESOURCE (m_nBitmapId));
	HBITMAP hBitmap = ::LoadBitmap (AfxGetInstanceHandle(), MAKEINTRESOURCE (m_nBitmapId));
	...
	GetClientRect (&rectButton);
	CPoint pt;
	if (m_bText || (m_type == CFooButton::checkBox) || (m_type == CFooButton::radio))
...
void CFooButton::drawCaption(CDC* pDC, LPDRAWITEMSTRUCT lpDrawItemStruct, int nLeftEdge, int& nRightEdge)
{
...
void CFooButton::drawDropDown(CDC* pDC, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	ASSERT (pDC != NULL);
	ASSERT (lpDrawItemStruct != NULL);
...
void CFooButton::drawMultiDropDownRegion(CDC* pDC, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	ASSERT (pDC != NULL);
	ASSERT (lpDrawItemStruct != NULL);
	if (!m_bMulti || !m_bMultiClicked)
...
void CFooButton::drawFocus(CDC* pDC, LPDRAWITEMSTRUCT lpDrawItemStruct, int nLeftEdge, int nRightEdge)
{
	ASSERT (pDC != NULL);
	ASSERT (lpDrawItemStruct != NULL);
	
	// Draw focus rectangle (for non-static non-hot buttons)
	if (!m_bStatic && !m_bHot && (getFocusStyle() != CFooButton::noFocus))
...
void CFooButton::drawDefaultBorder(CDC* pDC, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	ASSERT (pDC != NULL);
	ASSERT (lpDrawItemStruct != NULL);
	
	// Only pushbuttons can have a default border
	CFooButton::Type btnType = getType();
	if ((btnType != CFooButton::pushButton) &&
		(btnType != CFooButton::pushButtonDropDown) &&
		(btnType != CFooButton::pushButtonMulti))
		return;
	
	// Nothing to do if the default border isn't required
	if (getFocusStyle() != CFooButton::defaultFocus)
		return;
...
//    None
//
void CFooButton::TransparentBlt
...
void CFooButton::drawDropDownIndicator
(HDC      hdcDest,            // destination DC
...
void CFooButton::drawHotButtonFrame(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	// Get DC
	ASSERT (lpDrawItemStruct != NULL);
	CDC* pDC = CDC::FromHandle (lpDrawItemStruct->hDC);
...
void CFooButton::multiHitTest(CPoint pt)
{
...
void CFooButton::getButtonGroup(CString strGroupName, void* & pGroup)
{
	pGroup = NULL;
...
void CFooButton::drawGradientRect(CDC *pDC, CRect r, COLORREF cLeft, COLORREF cRight, BOOL bVertical)
{
