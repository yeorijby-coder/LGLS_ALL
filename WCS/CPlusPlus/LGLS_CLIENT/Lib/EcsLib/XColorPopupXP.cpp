// XColorPopupXP.cpp : implementation file
//
...
//
// CXColorPopupXP is a helper class for the color picker control
// CXColorPickerXP. Check out the header file or the accompanying
...
// Here is complete list of colors and color names:
//     http://msdn2.microsoft.com/en-us/library/aa358802(VS.85).aspx

ColorTableEntry CXColorPopupXP::m_crColors[] =
{
	// row 0
...
//=============================================================================
BEGIN_MESSAGE_MAP(CXColorPopupXP, CWnd)
//=============================================================================
	//{{AFX_MSG_MAP(CXColorPopupXP)
	ON_WM_NCDESTROY()
	ON_WM_LBUTTONUP()
	ON_WM_PAINT()
	ON_WM_MOUSEMOVE()
...
//=============================================================================
CXColorPopupXP::CXColorPopupXP(CWnd * pParentWnd /*= NULL*/,
...
//=============================================================================
{
	...
	m_bIgnoreLButtonUp = bIgnoreLButtonUp;
	...
	Create(point, crColor, pParentWnd, szCustomText);
}

//=============================================================================
void CXColorPopupXP::Init()
//=============================================================================
{
	TRACE(_T("in CXColorPopupXP::Init\n"));

	m_bFirstTime    = TRUE;
	m_bColorDialogIsDisplayed = FALSE;
...
//=============================================================================
CXColorPopupXP::~CXColorPopupXP()
//=============================================================================
{
	TRACE(_T("in CXColorPopupXP::~CXColorPopupXP\n"));
	if (m_Font.GetSafeHandle())
		m_Font.DeleteObject();
}

//=============================================================================
BOOL CXColorPopupXP::Create(CPoint p,
						 COLORREF crColor,
						 CWnd * pParentWnd,
						 LPCTSTR szCustomText /* = NULL */)
//=============================================================================
{
	...
	{
		TRACE(_T("ERROR CreateEx failed\n"));
		ASSERT(FALSE);
		return FALSE;
	}
...
BOOL CXColorPopupXP::PreTranslateMessage(MSG * pMsg)
//=============================================================================
{
	...
	{
		TRACE(_T("WM_SYSKEYDOWN: lParam=0x%X\n"), pMsg->lParam);
		LPARAM lParam = pMsg->lParam;
		UINT key = (UINT) (lParam >> 16);
		key = key & 0xFF;
...
void CXColorPopupXP::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
//=============================================================================
{
	TRACE(_T("in CXColorPopupXP::OnKeyDown\n"));
...
//=============================================================================
void CXColorPopupXP::OnNcDestroy()
//=============================================================================
{
	if (m_pParent)
		m_pParent->SetFocus();
...
//=============================================================================
void CXColorPopupXP::OnMouseMove(UINT nFlags, CPoint point)
//=============================================================================
{
	int row, col;
	...
	{
		row = CUSTOM_BUTTON_ROW;        // Special value meaning Custom button
		col = 0;
	}
...
void CXColorPopupXP::OnLButtonUp(UINT nFlags, CPoint point)
//=============================================================================
{
	TRACE(_T("in CXColorPopupXP::OnLButtonUp\n"));

	if (m_bFirstTime && m_bIgnoreLButtonUp)
	{
		// the first WM_LBUTTONUP message will be from user
		// letting go of mouse button on CXColorPickerXP 
		m_bFirstTime = FALSE;
		return;
	}
...
BOOL CXColorPopupXP::GetCellRect(int row, int col, const LPRECT & rect)
//=============================================================================
{
...
//=============================================================================
void CXColorPopupXP::SetWindowSize()
//=============================================================================
{
	CSize TextSize;
...
//=============================================================================
void CXColorPopupXP::CreateToolTips()
//=============================================================================
{
	// Create the tool tip
...
//=============================================================================
void CXColorPopupXP::ChangeSelection(int row, int col)
//=============================================================================
{
...
//=============================================================================
void CXColorPopupXP::EndSelection(int nMessage)
//=============================================================================
{
	ReleaseCapture();
	...
	else if (nMessage != CPN_SELENDCANCEL && m_nCurrentRow == CUSTOM_BUTTON_ROW)
	{
		...
		m_bColorDialogIsDisplayed = TRUE;
...
//=============================================================================
void CXColorPopupXP::DrawCell(CDC * pDC, int row, int col)
//=============================================================================
{
...
//=============================================================================
void CXColorPopupXP::OnSysChar(UINT nChar, UINT nRepCnt, UINT nFlags) 
//=============================================================================
{
...
//=============================================================================
void CXColorPopupXP::OnTimer(UINT nIDEvent) 
//=============================================================================
{
	KillTimer(nIDEvent);
	PostMessage(WM_KEYDOWN, VK_ESCAPE);	// simulate the ESC key
	//CWnd::OnTimer(nIDEvent);
}

//=============================================================================
void CXColorPopupXP::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
//=============================================================================
{
	if (nState == WA_INACTIVE)
	{
		TRACE(_T("in CXColorPopupXP::OnActivate: inactive\n"));
		if (!m_bColorDialogIsDisplayed)
		{
			PostMessage(WM_KEYDOWN, VK_ESCAPE);	// simulate the ESC key
		}
	}
...
//=============================================================================
void CXColorPopupXP::OnKillFocus(CWnd* pNewWnd) 
//=============================================================================
{
	TRACE(_T("in CXColorPopupXP::OnKillFocus\n"));
