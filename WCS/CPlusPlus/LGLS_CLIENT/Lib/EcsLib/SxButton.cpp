...
{
	m_hIcon = (HICON)::LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(nID), IMAGE_ICON, nWidth, nHeight, 0);
//	m_hIcon = (HICON)::LoadImage(AfxGetApp()->m_hInstance, MAKEINTRESOURCE(nID), IMAGE_ICON, nWidth, nHeight, 0);

//	TCHAR szName[1024];
//	::GetModuleFileName(AfxGetInstanceHandle(), szName, 1024);
...
BOOL CSxButton::SetBitmapCommon( UINT nID, int nWidth, int nHeight, COLORREF crTransparentMask, BOOL bUseMask )
{
	...
	HBITMAP bmTemp = (HBITMAP)::CreateMappedBitmap(AfxGetInstanceHandle(), nID, IMAGE_BITMAP, &mapColor, bUseMask ? 1 : 0 );
