...
BEGIN_MESSAGE_MAP(CReportHeader, CHeaderCtrl)
END_MESSAGE_MAP()

// CReportHeader 메시지 처리기입니다.
//
void CReportHeader::SetSortArrow(const int iColumn)
{
	m_bAscending = iColumn == m_nColumn ? !m_bAscending : TRUE;
	m_nColumn = iColumn;
	...
	VERIFY( GetItem( iColumn, &hditem ) );
	hditem.fmt |= HDF_OWNERDRAW;
	VERIFY( SetItem( iColumn, &hditem ) );
...
void CReportHeader::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	if( lpDrawItemStruct->itemID == (UINT)m_nColumn )
		rc.right -= 3 * iOffset;
