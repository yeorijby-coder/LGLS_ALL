...
void CColorButton::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	char c_szText[bufSize];
	wcstombs(c_szText, buffer, wcslen(buffer) + 1);
	DrawButtonText(pDC, btnRect, c_szText, GetFGColor());
	...
	if (state & ODS_FOCUS) {
		...
		DrawButtonText(pDC, btnRect, c_szText, GetDisabledColor());
	}
} 
