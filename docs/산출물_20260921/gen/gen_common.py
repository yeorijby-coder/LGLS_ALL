# -*- coding: utf-8 -*-
"""공통 도우미 : LGLS ECS 문서 산출물 생성용 (2026-09-21 최종판 : 현장 기본 시험 완료 기준)"""
import os, re
...
VER = '2.0'
# 문서 공통 이력. 문서마다 new_doc(history=[...]) 로 중간 판을 덧붙인다.
BASE_HISTORY = [['1.0', '2026-09-03', AUTHOR, '최초 작성'],
                ['1.1', '2026-09-04', AUTHOR, '검토 의견 반영 (숨김 메뉴·시뮬레이터 표기 제외)']]
FINAL_NOTE = '최종판 - 현장 기본 시험 완료 기준 전면 갱신 (미사용 설정·기능 정리, PLC 알람 리스트, C/V#2 통로 사용권)'


def new_doc(title, subtitle=None, landscape=False, history=None):
    d = Document()
    ...
    p = d.add_paragraph(); p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r = p.add_run('ECS Renewal   |   작성일 %s   |   Ver %s' % (DATE, VER)); r.font.size = Pt(10); r.font.color.rgb = GRAY
    d.add_page_break()
    d.add_heading('문서 이력', 1)
    rows = list(BASE_HISTORY if history is None else history) + [[VER, DATE, AUTHOR, FINAL_NOTE]]
    table(d, ['버전', '일자', '작성', '내용'], rows, widths=[1.5, 2.5, 3.0, 9.0])
    d.add_page_break()
    return d
...
def menu_states():
    ini = read_cp949(os.path.join(ROOT, 'EXE_NEWUI', 'WCS_CLIENT', 'Ecs.ini'))
    m = {}
