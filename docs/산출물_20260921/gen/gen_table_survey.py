...
#   2.0(2026-09-21) = 최종판 재발행. 내용(조사 결과·처리)은 Rev2 그대로, 문서 이력만 추가.
#   판정 기준 : "빌드되는 프로그램의 소스에 그 테이블을 대상으로 한 SQL 이 있는가"
#   행 수는 근거로 쓰지 않는다(eqp_err_his 처럼 아직 안 쌓였을 뿐인 표가 있다).
import os, io, sys, datetime
sys.path.insert(0, r'D:\project\LGLS\Renewal\docs\산출물_20260911\gen')
from docx import Document
from docx.shared import Pt, Cm, RGBColor
from docx.enum.text import WD_ALIGN_PARAGRAPH
from docx.enum.table import WD_TABLE_ALIGNMENT

OUT = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), '10_테이블_사용현황_조사표.docx')
...
d.add_heading('문서 이력', level=1)
htbl = d.add_table(rows=1, cols=4); htbl.style = 'Table Grid'; htbl.alignment = WD_TABLE_ALIGNMENT.CENTER
for i, c in enumerate(['버전', '일자', '작성', '내용']):
    cell = htbl.rows[0].cells[i]; cell.text = c
    for r in cell.paragraphs[0].runs: r.bold = True
for row in [['1.0', '2026-09-10', 'LGLS ECS Renewal', '최초 조사 (삭제 권고 7 / 판단 필요 5 / 유지 25)'],
            ['2.0(Rev2)', '2026-09-10', 'LGLS ECS Renewal', '판단 필요 5건도 폐지 결정, 12개 표 삭제 실행 반영'],
            ['2.0', '2026-09-21', 'LGLS ECS Renewal', '최종판 재발행 (내용은 Rev2 그대로, 산출물 폴더 갱신)']]:
    cells = htbl.add_row().cells
    for i, v in enumerate(row): cells[i].text = v
for i, w in enumerate([Cm(2.2), Cm(2.2), Cm(3.2), Cm(9.0)]):
    for row in htbl.rows: row.cells[i].width = w
d.add_paragraph()
