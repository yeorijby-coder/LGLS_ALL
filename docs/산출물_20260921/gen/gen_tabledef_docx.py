# -*- coding: utf-8 -*-
"""테이블 정의서 Rev4 : 미사용 표 12개까지 DB 에서 지운 뒤의 최종판.
   Rev3 = 미사용 컬럼 51개 삭제 (DB_BACKUP/01_컬럼정리.sql)
   Rev4 = 미사용 표 12개 삭제 (DB_BACKUP/11_표삭제.sql)
   ...
   남은 25 표를 하나도 빼지 않고 전부 싣는다(문서와 DB 가 1:1 로 맞아야 한다).
   컬럼 설명은 coldesc.py 로 전 컬럼을 채운다."""
import os, re
...
TDESC = OrderedDict([
 ...
 ('EQP_ECD_MST', ('설비 에러코드 마스터', '설비별 에러코드와 명칭. EQP_TYP 로 구분 : CV / RTV / SC(공통) / SC_SFA / SC_MURATA_EN / SC_RESP / SC_LGLS(2026-09-17 PLC 알람 리스트, 크레인 지상반 알람 전량 반영).')),
 ...
 ('CHG_LANG', ('다국어 문구', '운전 화면 문구 번역표. 화면 ini 의 4개 언어 섹션과 짝을 이룬다.')),
...
# 컬럼 설명(핵심)
CDESC = {
 'WH_TYP': '창고 구분(10 = LG 화학 1동)', 'LUGG_NO': '작업(화물) 번호 4자리', 'JOB_TYP': '작업 구분 1=입고 2=출고 3=피킹출고 4=랙투랙 5=호기간이동 6=이동 1x=반자동',
 'JOB_STATUS': '작업 상태(COMMON_CODE) 99 신규 10/15 CV 16 통로CV 구동중 20/25 SC 30/35 RGV 39 RGV완료 19/29 완료 09 이력이관', 'START_POS': '출발지(설비번호 : 트랙 1xx / 크레인 90x)', 'DEST_POS': '도착지(설비번호)',
 ...
 'EQP_TYP': 'EQP_MST: 설비 종류(EQP/HOST/HOST2/SCH/CV/SC/RTV). EQP_ECD_MST: CV/RTV/SC/SC_SFA/SC_MURATA_EN/SC_RESP/SC_LGLS(2026-09-17 PLC 알람 리스트)', 'PLC_IP': 'PLC IP', 'PLC_PORT': 'PLC 포트', 'CONNECTED_YN': '접속 여부(하트비트)', 'USE_YN': '사용 여부',
...
EXCLUDE = []   # Rev4 : DB 에 남은 표는 전부 싣는다
...
d = new_doc('테이블 정의서 (Rev4)', 'LGLS_MCS_IO (Microsoft SQL Server) - %d 테이블 · 전 컬럼 설명 포함  (2026-09-10 미사용 표 12개 삭제 반영)' % len(by),
...
para(d, '※ 현재 DB(LGLS_MCS_IO)에 있는 표 전부다. 문서에 없는 표는 DB 에도 없다.', color=GRAY, size=9)
para(d, '※ 2026-09-10 삭제한 표 12개 : auto_sc_work, bcr_mst, client_pgr_nm, dest_def, event_log, host_empty_plt, mes_if_log, rtv_data, sc_data, wait_track, wc_data, wc_his '
        '(근거 : 테이블_사용현황_조사표.docx / 삭제 스크립트 : DB_BACKUP/11_표삭제.sql, 되돌리기 13_표복구.sql)', color=GRAY, size=9)
d.add_page_break()
...
for t, cols in by.items():
    ...
    for c in cols:
        ...
        body.append([pos, col.upper(), typ2, 'N' if nul == 'NO' else 'Y', pk, dflt.strip('()'),
                     CDESC.get(col.upper()) or coldesc.describe(col)])
