...
pic(s, 'layout.png', 0.5, 1.2, 5.7, 5.6)
add_table(s, 6.4, 1.2, 6.5, [
    ['동작', '대상', '결과'],
    ['클릭', 'C/V 트랙', 'CV 상태 창'],
...
bullets(s, 6.4, 3.6, 6.5, 3.3, '■ 입출고 모드 전환 (더블클릭)', [
    ...
    '트랙 글자는 리본 [창고 모니터링] 선택(작업번호/트랙번호/제품정보)을 따른다.',
    '작업대 명칭은 해당 트랙 왼쪽에 둔다 : 103 제품 입고대(TR#24) · 104 원부자재불출대(TR#26) · 102 피킹 작업대(출고대 TR#29 / 입고대 TR#30) · [101] 입출고대(TR#22).',
    '명칭 글자색 : 입고대 = 범례 입고색, 출고대 = 범례 출고색(범례 변경 시 함께 바뀜), 101 입출고대 = 핑크 고정.'], 10.5)
footer(s)
...
add_table(s, 0.5, 1.2, 12.4, [
    ...
    ['11', '작업대 명칭', '위치 정정(103=TR#24 입고대, 104=TR#26 출고대), 102 를 출고대(TR#29)/입고대(TR#30) 둘로 분리, 글자색 = 범례 입고/출고색, 101 = 핑크'],
