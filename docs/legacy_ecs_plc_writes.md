...
### 역방향 계산 (PPT 표기 M/D/R → 실주소 → ezMCS 표기 B/W/R)

| 영역 | 정방향 (표기→실주소) | 역방향 (실주소→표기) |
|---|---|---|
| **M(비트)** | `%MX = 워드×16 + 비트` (M492 = 워드49·비트2) | PPT: `워드=%MX÷16, 비트=%MX%16` / B표기: `%MX를 16진` |
| **D(워드)** | `160~: 256+(n-160)/10×16+n%10`<br>`300~: 480+(n-300)`<br>`320~: 768+(n-320)/10×16+n%10`<br>그 외: `%DW = n` | PPT: `≥768: 320+(w-768)/16×10+(w-768)%16`<br>`≥480: 300+(w-480)`<br>`≥256: 160+(w-256)/16×10+(w-256)%16`<br>W표기: `w를 16진` |
| **R** | `워드 = R번호(10진)`, `%RB = 워드×2` | `워드 = %RB÷2`, R번호 = 워드(동일) |

역산 예:

| PPT 표기 | 실주소 | ezMCS 표기 | 실존 태그 |
|---|---|---|---|
| `M492` (워드49·비트2) | %MX786 | `B0312` | VEHICLE:11 `ALARM_SET_REPORT` ✓ (CvThread.cs:3317 `M_ALARM_SET = V09 ? 492 : 786` 과 일치) |
| `D101` (세그먼트 밖) | %DW101 | `W0065` | 없음(스페어) |
| `R100` | 워드100, %RB200 | `R0100` | CONVEYOR:11 `PALLET_EXIST01` ✓ |

**⚠ 표기 혼동 주의**: `D101`(PPT·10진) = %DW101 이지만, `W0101`(ezMCS·**16진**) = 0x101 = 257 = %DW257 = PPT `D0161`
(= VEHICLE:11 `ALARM_SET_CODE`). 같은 "101" 이라도 전혀 다른 주소다.

### 워드 표기 / 바이트 표기 병기

단위 사다리: **비트 ─(÷16)→ 워드 ─(×2)→ 바이트** (역방향은 ×16, ÷2).
M(비트영역)만 3단위 모두 쓰고, D·R(워드영역)은 워드·바이트 2단위만 쓴다.

| 구표기 | 10진 변환 | 비트 표기 | 워드 표기 | 바이트 표기 | 실존 태그 |
|---|---|---|---|---|---|
| `B312` | 0x312 = 786 | `%MX786` | `%MW49` 의 비트 2 | `%MB98` 의 비트 2 | VEHICLE:11 `ALARM_SET_REPORT` ✓ (PPT: M0492) |
| `W0065` | 0x65 = 101 | — | `%DW101` | `%DB202` | 없음(스페어). PPT D표 범위 밖 — 표 시작은 W0100=%DW256=D0160 |
| `R100` | 10진 그대로 100 | — | 워드 100 (`%RW100`) | `%RB200` | CONVEYOR:11 `PALLET_EXIST01` ✓ (LENGTH=2 → 워드100~101 = %RB200~203) |

`%MW49` = `%MB98~99` = `%MX784~799`(16비트) 이므로, 786 은 그 워드의 비트 2 이자 바이트 98 의 비트 2.
