...
para(d, '기동 순서 : 설비 통신 → 스케줄러 → 상위 통신 (시험 시에는 EQP_SIM → 설비 통신 → 스케줄러 → HOST_SIM → 상위 통신 → HOST_SIM [시작]). 운전 화면은 아무 때나.')
para(d, '작업 상태 코드 : 99 신규 → 10/11/15 CV 구동대기/구동중 → 30/31/35 RGV → 39 RGV 완료 → 20/21/25 SC → 29 SC 완료 → 09 완료. 출고는 SC 부터 시작해 CV 완료(19)로 끝난다.')
d.add_page_break()
...
for g, name, sh, path, overview, notes, ctrls in SCREENS:
    ...
    else: para(d, '(현재 설정에서 숨김이거나 자동 캡처가 되지 않는 화면 - 설명으로 대체)', italic=True, color=GRAY, indent=0.5)
    if notes:
...
table(d, ['상황', '처리'], [
    ...
    ['이중입고(54)/공출고(58) 에러', 'SC 상태창 [에러리셋] 후 상위(WMS) 재지정을 기다린다(자동 재지정 흐름 구현됨)'],
