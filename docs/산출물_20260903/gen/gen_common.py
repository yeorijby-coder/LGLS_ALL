...
OUT = r'D:\project\LGLS\Renewal\docs\산출물_20260903'
SHOTS = r'C:\Users\USER\AppData\Local\Temp\claude\D--project-LGLS-Renewal\4609e0ee-85bd-49e7-8c4e-b9231f6fb7f0\scratchpad\shots'
...
def new_doc(title, subtitle=None, landscape=False):
    ...
    p = d.add_paragraph(); p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r = p.add_run('WCS Renewal (구 ECS 대체)   |   작성일 %s   |   Ver 1.0' % DATE); r.font.size = Pt(10); r.font.color.rgb = GRAY
    d.add_page_break()
    # 문서 이력
    d.add_heading('문서 이력', 1)
    t = table(d, ['버전', '일자', '작성', '내용'], [['1.0', DATE, AUTHOR, '최초 작성 (시뮬레이터 순환 시험 완료 시점 기준)']], widths=[1.5, 2.5, 3.0, 9.0])
    d.add_page_break()
    return d
...
def task_view_states():
    r = {}
    for name, path in (('설비 통신(WCS_TASK_CV)', r'D:\project\LGLS\Renewal\TASK\WCS_TASK_CV_BIN\bin\Debug\WCS_DB.INI'),
                       ('스케줄러(IO_TASK)', r'D:\project\LGLS\Renewal\TASK\IO_TASK\bin\Debug\ENV_IOSCH.INI')):
        t = read_cp949(path); mm = re.search(r'LOG_FILTER_BTN\s*=\s*(\w)', t)
        r[name] = (mm.group(1).upper() == 'Y') if mm else True
    return r
