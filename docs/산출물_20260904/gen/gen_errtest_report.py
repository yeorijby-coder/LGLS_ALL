...
para(d, '※ 본 시험은 규격상 정의된 R 전문(R_Kind=2) 재지정 경로로 12 회 검증했다. '
        'IMS 담당자 확인 결과 현장에서는 공출고 재지정을 쓰지 않고 위와 같이 삭제하므로, '
        '시뮬레이터(HOST_SIM)의 버튼도 [공출고 작업 삭제] 로 바꾸었다. '
        'WCS 의 R_Kind=2 처리는 규격 지원으로 남겨 두었다(상위가 보내지 않으면 동작하지 않는다).',
     size=9, color=GRAY)
