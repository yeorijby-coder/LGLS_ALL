...
def footer(slide):
    page[0] += 1
    tb(slide, 0.5, 7.0, 4, 0.35, 'LG 화학 1동 ECS  |  화면설계서', 10, False, GRAY)
...
tb(s, 1.0, 4.6, 11, 0.6, 'ECS Renewal   ·   Ver 1.1   ·   %s' % DATE, 16, False, RGBColor(0xCA, 0xDC, 0xFC))
...
add_table(s, 0.7, 1.4, 11.9, [['Vers.', 'Date', 'Author', 'Approver', 'Notes'], ['V1.0', '2026-09-03', 'LGLS ECS Renewal', '', '최초 작성'], ['V1.1', DATE, 'LGLS ECS Renewal', '', '명칭 ECS 확정, 범례 화면 내장, 판넬 보기 신설, 캡처 갱신']], [1.2, 1.8, 2.6, 1.8, 4.5])
footer(s)
...
out = os.path.join(OUT, '06_ECS_화면설계서.pptx'); prs.save(out); print('saved', out, 'slides', len(prs.slides))
