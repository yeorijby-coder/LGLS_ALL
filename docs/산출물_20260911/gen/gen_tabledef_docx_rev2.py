# -*- coding: utf-8 -*-
"""테이블 정의서 Rev2 : 컬럼 설명 보강판 (coldesc.py 로 전 컬럼 설명)
   기존판(03_테이블_정의서.docx)과 비교하도록 별도 파일로 저장한다."""
import os, re
...
d = new_doc('테이블 정의서 (Rev2)', 'LGLS_MCS_IO (Microsoft SQL Server) - %d 테이블 · 전 컬럼 설명 포함' % len(by))
...
out = os.path.join(OUT, '03_테이블_정의서_Rev2.docx'); d.save(out); print('saved', out, 'tables', len(by))
