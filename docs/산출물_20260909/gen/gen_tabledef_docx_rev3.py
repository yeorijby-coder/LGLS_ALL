# -*- coding: utf-8 -*-
"""테이블 정의서 Rev3 : 이 현장 미사용 컬럼 51개를 DB 에서 지운 뒤의 최종판.
   (cv_data 39 / job_mst 6 / job_mst_his 6 - 2026-09-10 DB_BACKUP/01_컬럼정리.sql)
   컬럼 설명은 Rev2 와 같이 coldesc.py 로 전 컬럼을 채운다."""
import os, re
...
d = new_doc('테이블 정의서 (Rev3)', 'LGLS_MCS_IO (Microsoft SQL Server) - %d 테이블 · 전 컬럼 설명 포함' % len(by))
...
out = os.path.join(OUT, '03_테이블_정의서_Rev3.docx'); d.save(out); print('saved', out, 'tables', len(by))
