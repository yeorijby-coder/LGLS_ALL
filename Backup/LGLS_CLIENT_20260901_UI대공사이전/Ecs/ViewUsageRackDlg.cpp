...
BEGIN_MESSAGE_MAP(CViewUsageRackDlg, CSkinDialog)
	ON_MESSAGE(SSM_CLICK, &CViewUsageRackDlg::OnLglsHdrSort)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_SIZE()
	ON_WM_CLOSE()
...
// [LGLS] 그리드 헤더 클릭 정렬
LRESULT CViewUsageRackDlg::OnLglsHdrSort(WPARAM wParam, LPARAM lParam)
{
	LPSS_CELLCOORD pCell = (LPSS_CELLCOORD)lParam;
	if(pCell != NULL && pCell->Row == 0 && pCell->Col >= 1)
		m_pSpdCellList.SortByHeaderColumn(pCell->Col);
	return 0;
}
