...
BEGIN_MESSAGE_MAP(CFireMessageDlg, CSkinDialog)
	ON_MESSAGE(SSM_CLICK, &CFireMessageDlg::OnLglsHdrSort)
	ON_WM_CLOSE()
...
// [LGLS] 그리드 헤더 클릭 정렬
LRESULT CFireMessageDlg::OnLglsHdrSort(WPARAM wParam, LPARAM lParam)
{
	LPSS_CELLCOORD pCell = (LPSS_CELLCOORD)lParam;
	if(pCell != NULL && pCell->Row == 0 && pCell->Col >= 1)
		m_spd.SortByHeaderColumn(pCell->Col);
	return 0;
}
...
void CFireMessageDlg::GetCellNo()
{
	...
	strSql.Format(_T("	SELECT CELL_SC_NO, CELL_NO, SUBSTRING(AGING_TYP,1,3) AS AGING_TYP  \n") // [LGLS] SUBSTR -> SUBSTRING
     			  _T("    FROM CELL_MST													\n")
				  _T("   WHERE WH_TYP = '%s'											\n")
...
void CFireMessageDlg::RenameResource( EN_LANG m_enLang)
{
	...
	if (!strValue.IsEmpty()) SetDlgItemText(IDC_BTN_STOP, strValue);
}
