...
{
    public delegate void WidgetEventDelegate01(object sender, EventArgs e);

    public partial class MonitorAllEquipmentPanel01 : UserControl
    {
        ...
        public MonitorAllEquipmentPanel01()
        {
            InitializeComponent();
            RegisterEvent();
            LinkingDevice(this);
        ...
        private void RefreshConveyor(Conveyor cnv)
        {
            foreach (Port port in cnv.Ports.Values)
            {
                try
                {
                    string palletid = cnv.GetPalletFromPLC(port);
                }
                catch (Exception ex)
                {
                }
