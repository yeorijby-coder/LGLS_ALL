...
{
    [Serializable]
    public class TransportDevice : Equipment
    {
        const string CLASS = "TransportDevice";
        ...
        public TransportDevice(string equipment)
            : base(equipment)
        {
        }

        public TransportDevice(string equipment, EquipmentType equipmentType,
        ...
        public TransportDevice(string equipment, EquipmentType equipmentType, string hostId,
            string assemblyName, string className)
            : base(equipment, equipmentType, hostId, assemblyName, className)
        {
        ...
        protected override void Add(DBConnection theDB)
        {
            ...
            try
            {
                ...
                //if (reader.Read())
                //{
                //    Log.log(5, METHOD, "debug", Category.DEBUG, "", "transport", this.elementId + "already Exist");
                ...
                //    theDB.ExecuteUpdate("insert into TB_EQUIPMENT (EQUIPMENTID) values (@EQUIPMENTID)", pss);
            ...
            catch (SqlException ex)
            {
                ...
                Log.log(5, METHOD, "error", Category.ERROR, "", "transport", "sql error \n " + ex.Message.ToString());
                throw ex;
            }
            catch (Exception ex)
            {
        ...
        protected override void Load(DBConnection theDB)
        {
            ...
            try
            {
                ...
                {
                    Log.log(5, METHOD, "debug", Category.DEBUG, "", "transport", this.elementId + "doesnot exist");
                }
            }
            catch (SqlException ex)
            {
                StatisticsCollector.TriggerOccured(
                                                  RunTimeExceptionStatisticsCollectorData.COLLECTOR_ID,
                                                  RunTimeExceptionStatisticsCollectorData.TRIGGER_CORE_EXCEPTION,
                                                  new RunTimeExceptionStatisticsCollectorData("TransportDevice", "Load", "Loading SQL Error", ex.ToString(), ex.StackTrace.ToString()));
                Log.log(5, METHOD, "error", Category.ERROR, "", "transport", "sql error \n " + ex.Message.ToString());
                throw ex;
            }
            catch (Exception ex)
            {
                StatisticsCollector.TriggerOccured(
                                                  RunTimeExceptionStatisticsCollectorData.COLLECTOR_ID,
                                                  RunTimeExceptionStatisticsCollectorData.TRIGGER_CORE_EXCEPTION,
                                                  new RunTimeExceptionStatisticsCollectorData("TransportDevice", "Load", "Loading Error", ex.ToString(), ex.StackTrace.ToString()));
                Log.log(5, METHOD, "error", Category.ERROR, "", "transport", "error \n " + ex.Message.ToString());
                throw ex;
            }
            finally
            {
        ...
        public Conveyor GetConveyor(string key)
        {
            ...
            foreach (Conveyor cnv in _conveyors.Values)
            {
                ...
                else if (string.IsNullOrEmpty(cnv.ElementId) == false && cnv.ElementId == key)
                {
                    return cnv;
                }
                else if (string.IsNullOrEmpty(cnv.Id) == false && cnv.Id == key)
                {
                    return cnv;
                }
            }
        ...
        public Vehicle GetVehicle(string key)
        {
            ...
            foreach (Vehicle vehicle in _vehicles.Values)
            {
                ...
                else if (string.IsNullOrEmpty(vehicle.Id) == false && vehicle.Id == key)
                {
                    return vehicle;
                }
                else if (string.IsNullOrEmpty(vehicle.ElementId) == false && vehicle.ElementId == key)
                {
                    return vehicle;
                }
            }
