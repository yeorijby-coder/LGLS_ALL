...
namespace ECP.Util
{
    public class StreamTokenizer
    {
        ...
        private static readonly int NEED_CHAR = Int32.MaxValue;
        private static readonly int SKIP_LF = Int32.MaxValue - 1;
         ...
         *
         * @see     java.io.StreamTokenizer#eolIsSignificant(bool)
        ...
        public int NextToken() 
        {
	        ...
	        int _ctype = c < 256 ? ct[c] : CT_ALPHA;

	        while ((_ctype & CT_WHITESPACE) != 0) 
            {
	            if (c == '\r') {
		        LINENO++;
	            ...
	            if (c < 0)
		        return ttype = TT_EOF;
	            _ctype = c < 256 ? ct[c] : CT_ALPHA;
	        }

	        if ((_ctype & CT_DIGIT) != 0) 
            {
	            bool neg = false;
	            if (c == '-') 
                {
	        ...
	        if ((_ctype & CT_ALPHA) != 0) 
            {
	            ...
	            do 
                {
		            ...
		            buf[i++] = (char) c;
		            c = Read();
		            _ctype = c < 0 ? CT_WHITESPACE : c < 256 ? ct[c] : CT_ALPHA;
	            } while ((_ctype & (CT_ALPHA | CT_DIGIT)) != 0);
	            peekc = c;

                sval = new String(buf, 0, i);
