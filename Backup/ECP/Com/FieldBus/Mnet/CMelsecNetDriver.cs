...
{
    /// <summary>
    /// CMelsecNetDriver에 대한 요약 설명입니다.
    /// </summary>
    public partial class CMelsecNetDriver : AbstractFieldBusDriver
    {
        ...
        private int m_nMonitorInterval = 100;
        ...
        private int RefreshMonitorMemory()
        {
            ...
            jmp:
                try
                {
                    foreach (CMelsecNetMemory mem in m_MonitorMemory.Values)
                    {
        ...
        */
        ...
        {
            this.connectionInfo = (DriverConnectionString)new CMelsecNetConnection();
            ...
            for (int i = 0; i < DeviceTypeNames.Length; i++)
            {
                    ...
                    this.connectionInfo.ContainsKey(string.Format("{0}_END_ADDR", DeviceTypeNames[i])))
                {
                    m_MonitorMemory.Add(DeviceTypeNames[i],
                                        new CMelsecNetMemory((enumDeviceType)Enum.Parse(typeof(enumDeviceType), DeviceTypeNames[i]),
                ...
                case DataType._BOOL_ARRAY:
                    {
                        ...
                        CMelsecNetMemory mem = m_MonitorMemory[observable.ConnectionInfo["DEVICE_TYPE", "B"]];
                        short sAddressNo = (short)observable.ConnectionInfo["ADDRESS_NO", 0, 16];
                        short sBitIndex = (short)observable.ConnectionInfo["BIT_INDEX", 0];
