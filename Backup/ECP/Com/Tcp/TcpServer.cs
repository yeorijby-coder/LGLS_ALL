...
namespace ECP.Com.Tcp
{
    public class TcpServer
    {
        ...
        public bool MListening
        {
            get { return mListening; }
            set { mListening = value; }
        }
        #endregion
        ...
        private void OnDataReceived(IAsyncResult asyn)
        {
                ...
                else
                {
                    ...
                    for (int i = 0; i < buffer.Count; i++)
                    {
                        if (buffer[i] == (char)3)   //'0'를 ETX로 변경하세요.
                        {
                            ...
                            if (OnRead != null)
                            {
                                OnRead(socketData.m_currentSocket, Encoding.ASCII.GetString(messageTemp));
                            }
                            checkPoint = i + 1;
                        }
                    }
            ...
            catch (InvalidOperationException ex)
            {
                ...
                System.Console.Write("InvalidOperationException:" + ex.StackTrace);
