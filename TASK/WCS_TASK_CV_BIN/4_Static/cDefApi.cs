...
public static bool GsReadInitProfileDAddrDoc()
        ...
        /// <summary>
        /// [LGLS 2026-09-03] 메인 화면 [로그 필터] 버튼 표시 여부.  [VIEW] LOG_FILTER_BTN = Y | N
        ///   INI 없음 / 키 없음 / 빈 값 이면 기본 표시(Y). "N" 일 때만 숨긴다.
        /// </summary>
        public static bool GsReadInitProfileLogFilterBtn()
        {
            if (!System.IO.File.Exists(cDefApp.GM_ENV_INI))
                return true;
            StringBuilder sb = new StringBuilder(16);
            GetPrivateProfileString("VIEW", "LOG_FILTER_BTN", "Y", sb, sb.Capacity, cDefApp.GM_ENV_INI);
