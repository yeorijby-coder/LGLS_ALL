        ...
        public bool Connect(ref string msg)
        {
            ...
            try
            {
                if (!m_bSocCon)
                {
                    ...
                    // TCP KeepAlive: 유휴 5초 후 1초 간격 프로브 (유휴 접속 정리/중간장비 끊김 조기 감지)
                    try
                    {
                        _Socket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.KeepAlive, true);
                        byte[] keepAlive = new byte[12];
                        BitConverter.GetBytes((uint)1).CopyTo(keepAlive, 0);      // on
                        BitConverter.GetBytes((uint)5000).CopyTo(keepAlive, 4);   // idle ms
                        BitConverter.GetBytes((uint)1000).CopyTo(keepAlive, 8);   // interval ms
