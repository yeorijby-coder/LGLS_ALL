...
namespace WCS_TASK_CV
{
    ...
    public class cCbBasePost
    {
        ...
        public DbConn mCnMain;                                // [LGLS] @.외부에서 할당해주는 Connection 개체 참조(Connection이 하나 일 경우)
        public DbTran mTrnMain;                              // [LGLS] @.자체 생성되는 DB 객체들
        public DbCmd mComMain = new DbCmd();             // [LGLS] @.자체 생성되는 DB 객체들
        public DbAdap mDaMain = new DbAdap();      // [LGLS] @.Data Adapter
        ...
        public cCbBasePost(ref DbConn pConObj) // [LGLS]
        {
            this.mBindingType = false;
            this.mCnMain = pConObj;
            this.Init();
        }
        public cCbBasePost(ref DbConn pConObj, bool bBind) // [LGLS]
        {
            this.mBindingType = bBind;
            this.mCnMain = pConObj;
            this.Init();
        ...
        public void DbBaseOra(ref DbConn pConObj) // [LGLS]
        {
            this.mBindingType = false;
            this.mCnMain = pConObj;
            this.Init();
        }
        public void DbBaseOra(ref DbConn pConObj, bool bBind) // [LGLS]
        {
            this.mBindingType = bBind;
            this.mCnMain = pConObj;
            this.Init();
        ...
        {
            this.mBindingType = bBind;
            DbConn pConObj = new DbConn(pStringCon); // [LGLS]
            try
            {
                pConObj.Open();
                mCnMain = pConObj;
        ...
        #region ConvQry : :param / 방언 변환 (SqlClient 는 @param) // [LGLS]
        ...
        public int ExcuteQry(string strQry, bool bMsgBox, bool breturnErr)
        {
            ...
            try
            {
                ...
                mComMain.CommandText = ConvQry(strQry); // [LGLS]
                return mDaMain.Fill(mDtMain);
            }
            catch (DbExc DbErr) // [LGLS]
            {
                if (breturnErr)
                {
                    throw DbErr;
...
#if SQL
                mComMain.Transaction = mTrnMain; // [LGLS] SqlClient는 트랜잭션 진행중 커맨드에 Transaction 지정 필수
#endif
                mIsBeginTran = true;
            }
